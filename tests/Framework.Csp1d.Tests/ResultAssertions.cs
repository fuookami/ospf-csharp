...
public static class ResultAssertions {
    ...
    public static T ValueOrFail<T>(Result<T, ErrorCode, Error<ErrorCode>> result) {
        ...
        string message = result is Failed<T, ErrorCode, Error<ErrorCode>> f ? f.Message : "Result is not Ok";
    ...
    /// </summary>
    public static void AssertFailed<T>(Result<T, ErrorCode, Error<ErrorCode>> result) => Assert.True(result.IsFailed, "Expected Failed but got Ok");

    /// <summary>
    /// 断言 Result 为 Ok / Assert Result is Ok.
    /// </summary>
    public static void AssertOk<T>(Result<T, ErrorCode, Error<ErrorCode>> result) => Assert.True(result.IsOk, "Expected Ok but got Failed");
}
