...
public class CuttingPlanCanonicalKeyTest {
    ...
    public void FullKey_ShouldPreserveFields() {
        CuttingPlanSliceCanonicalKey[] slices = new[]
        {
            new CuttingPlanSliceCanonicalKey("product", "p-1", "100:MM", new UInt64(3UL))
        };
        CuttingPlanDemandContributionCanonicalKey[] contributions = new[]
        {
    ...
    public void DistinctByCanonicalKey_ShouldDeduplicate() {
        CuttingPlanCanonicalKey[] keys = new[]
        {
        ...
        List<CuttingPlanCanonicalKey> distinct = keys.DistinctByCanonicalKey(k => k);
