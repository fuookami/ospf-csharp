...
public class CrossTests {
    ...
    public void Cross2_ReturnsCorrectPairs() {
        List<(int, int)> result = Cross.Cross2(new[] { 0, 1 }, new[] { 2, 3 });
    ...
    public void Cross3_ReturnsCorrectCount() {
        List<(int, int, int)> result = Cross.Cross3(new[] { 0, 1 }, new[] { 2, 3 }, new[] { 4, 5 });
    ...
    public async Task CrossAsync_ReturnsSameCountAsSync() {
        ...
        int syncCount = Cross.CrossProduct(input).Count;

        int asyncCount = 0;
        await foreach (List<int> _ in Cross.CrossAsync(input).GetEnumerator()) {
            asyncCount++;
        }
