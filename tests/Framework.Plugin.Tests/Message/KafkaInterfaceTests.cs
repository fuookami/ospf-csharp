...
namespace Fuookami.Ospf.Framework.Plugin.Tests.Message
{
    ...
    {
        [Fact]
        public void KafkaProducer_Should_Implement_IDisposable()
        {
            typeof(IDisposable).IsAssignableFrom(typeof(KafkaProducer)).Should().BeTrue();
        }

        [Fact]
        public void KafkaConsumer_Should_Implement_IDisposable()
        {
            typeof(IDisposable).IsAssignableFrom(typeof(KafkaConsumer)).Should().BeTrue();
        }

        [Fact]
        public void KafkaProducer_Should_Have_Send_Method()
        {
            var method = typeof(KafkaProducer).GetMethod("Send");
        ...
        {
            var method = typeof(KafkaProducer).GetMethod("SendJson");
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(KafkaProducer).GetMethod("SendProtobuf");
            method.Should().NotBeNull();
        }
        ...
        {
            var methods = typeof(KafkaConsumer).GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
        ...
        {
            var method = typeof(KafkaConsumer).GetMethod("Consume");
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(KafkaConsumer).GetMethod("ConsumeAsync");
            method.Should().NotBeNull();
        }
    }
}
