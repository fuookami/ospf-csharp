...
namespace Fuookami.Ospf.Framework.Plugin.Tests.Persistence
{
    ...
    public class InterfaceVerificationTests
    {
        ...
        {
            var type = typeof(IPersistenceApiController);
            type.Should().NotBeNull();
            type.IsInterface.Should().BeTrue();
        }
        ...
        {
            var type = typeof(MongoRepository<>);
            type.Should().NotBeNull();
        ...
        {
            var type = typeof(SqliteRepository<>);
            type.Should().NotBeNull();
            type.IsAbstract.Should().BeTrue();
            type.IsGenericType.Should().BeTrue();
        }
        ...
        {
            var method = typeof(IRepository<object>).GetMethod("FindByIdAsync");
        ...
        {
            var prop = typeof(IPersistenceApiController).GetProperty("MongoClient");
