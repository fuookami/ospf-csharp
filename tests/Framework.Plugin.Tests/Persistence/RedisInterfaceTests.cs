...
namespace Fuookami.Ospf.Framework.Plugin.Tests.Persistence
{
    ...
    public class RedisInterfaceTests
    {
        ...
        {
            typeof(IDisposable).IsAssignableFrom(typeof(RedisClient)).Should().BeTrue();
        }

        [Fact]
        public void RedisClient_Should_Have_Set_Methods()
        {
            var methods = typeof(RedisClient).GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
        ...
        {
            var method = typeof(RedisClient).GetMethod("Get");
        ...
        {
            var method = typeof(RedisClient).GetMethod("GetList");
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(RedisClient).GetMethod("GetSet");
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(RedisClient).GetMethod("GetMap");
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(Redis).GetMethod("Init");
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(Redis).GetMethod("Get", new[] { typeof(RedisClientKey) });
            method.Should().NotBeNull();
        }
        ...
        {
            var method = typeof(Redis).GetMethod("GetByName");
            method.Should().NotBeNull();
        }
