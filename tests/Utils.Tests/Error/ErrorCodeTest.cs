...
namespace Fuookami.Ospf.Utils.Tests.Error
{
    ...
    public class ErrorCodeTest
    {
        ...
        [Fact]
        public void FromByte_InvalidCode_ReturnsUnknown()
        {
            ErrorCodeExtensions.FromByte(0x42).Should().Be(ErrorCode.Unknown);
        }

        [Fact]
        public void ToByte_RoundTrip()
        {
            foreach (var code in new[] { ErrorCode.None, ErrorCode.AuthenticationError, ErrorCode.ORModelInfeasible, ErrorCode.Other, ErrorCode.Unknown })
            {
        ...
        [Fact]
        public void ToUInt64_RoundTrip()
        {
            ErrorCodeExtensions.FromUInt64(ErrorCode.ORModelInfeasible.ToUInt64()).Should().Be(ErrorCode.ORModelInfeasible);
        }
