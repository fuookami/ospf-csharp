...
using Xunit;
using ER = Fuookami.Ospf.Utils.Functional.ExResult<int, Fuookami.Ospf.Utils.Error.ErrorCode, Fuookami.Ospf.Utils.Error.Error<Fuookami.Ospf.Utils.Error.ErrorCode>>;
using R = Fuookami.Ospf.Utils.Functional.Result<int, Fuookami.Ospf.Utils.Error.ErrorCode, Fuookami.Ospf.Utils.Error.Error<Fuookami.Ospf.Utils.Error.ErrorCode>>;
using RS = Fuookami.Ospf.Utils.Functional.Result<Fuookami.Ospf.Utils.Functional.Success, Fuookami.Ospf.Utils.Error.ErrorCode, Fuookami.Ospf.Utils.Error.Error<Fuookami.Ospf.Utils.Error.ErrorCode>>;
...
public class ResultTest {
    ...
    public void Map_Ok_TransformsValue() {
        R result = Results.Ok(21).Map(x => x * 2);
        result.IsOk.Should().BeTrue();
        result.Value.Should().Be(42);
    }
    ...
    public void Map_Failed_PropagatesError() {
        ...
        R result = failed.Map(x => x * 2);
        result.IsFailed.Should().BeTrue();
    }

    [Fact]
    public void Bind_Ok_ChainsOperations() {
        R result = Results.Ok(21).Bind<int>(x => Results.Ok(x * 2));
        result.IsOk.Should().BeTrue();
        result.Value.Should().Be(42);
    }
    ...
    public void Bind_Failed_ShortCircuits() {
        ...
        R result = failed.Bind<int>(x => Results.Ok(x * 2));
        result.IsFailed.Should().BeTrue();
    }

    [Fact]
    public void Success_Instance_IsSingleton() {
        Success s1 = Results.SuccessInstance;
        Success s2 = Results.SuccessInstance;
    ...
    public void LINQ_SelectMany_WorksCorrectly() {
        R result = from a in Results.Ok(10)
