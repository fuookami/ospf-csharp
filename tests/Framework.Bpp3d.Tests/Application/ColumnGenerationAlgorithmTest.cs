...
public class ColumnGenerationAlgorithmTest {
    ...
    public async Task AlgorithmShouldStopWhenNoAcceptedColumns() {
        ...
        Result<ColumnGenerationResult<FltX>, ErrorCode, Error<ErrorCode>> result = await algorithm.SolveAsync(
            items: new List<Item>(),
            config: ColumnGenerationConfig.Default with { FinalMilpEnabled = false });

        ColumnGenerationResult<FltX> unwrapped = result.Unwrap();
    ...
    public async Task AlgorithmShouldCompleteLpSpAddColumnAndFinalMilpFlow() {
        ...
        BinLayer seedLayer = MakeLayer("seed");
        BinLayer generatedLayer = MakeLayer("generated");

        var rmpCallStates = new List<(int Iteration, int ColumnCount)>();
        int finalSolverColumnCount = -1;
    ...
    public async Task RmpSolverAndRequestBuilderShouldBePreferredWhenProvided() {
        int fallbackRmpCounter = 0;
        int rmpSolverCounter = 0;
        int requestBuilderCounter = 0;
        int observedMaxCandidates = -1;
    ...
    public async Task AlgorithmShouldInvokeFinalSolveAnalyzerAndHeartbeatCallbacks() {
        ...
        Result<ColumnGenerationResult<FltX>, ErrorCode, Error<ErrorCode>> result = await algorithm.SolveAsync(items: new List<Item>());
    ...
    public async Task ApplicationServiceShouldSupportModelDemandEntryPoint() {
        ...
        BinLayer seedLayer = MakeLayer("svc-seed");
        ...
        Result<ColumnGenerationApplicationResponse, ErrorCode, Error<ErrorCode>> response = await service.SolveAsync(request);

        ColumnGenerationApplicationResponse unwrapped = response.Unwrap();
