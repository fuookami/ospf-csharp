...
public class ShapeTest {
    ...
    public void Shape2_IndexAndVector() {
        ...
        Result<int, ErrorCode, Error<ErrorCode>> indexResult = shape.Index(new[] { 1, 2 });
        indexResult.IsOk.Should().BeTrue();
        indexResult.Value.Should().Be(6); // 1*4 + 2*1 = 6

        Result<int[], ErrorCode, Error<ErrorCode>> vectorResult = shape.Vector(6);
    ...
    public void Shape3_WithColumnMajor() {
        ...
        Result<int, ErrorCode, Error<ErrorCode>> indexResult = shape.Index(new[] { 1, 2, 1 });
        indexResult.IsOk.Should().BeTrue();
    ...
    public void Shape4_BasicOperations() {
        ...
        Result<int, ErrorCode, Error<ErrorCode>> indexResult = shape.Index(new[] { 1, 2, 3, 4 });
        indexResult.IsOk.Should().BeTrue();
    ...
    public void Shape_IndexOutOfBounds_ReturnsFailed() {
        var shape = Shape2.Invoke(3, 4);
        Result<int, ErrorCode, Error<ErrorCode>> result = shape.Index(new[] { 5, 0 });
    ...
    public void Shape_DimensionMismatch_ReturnsFailed() {
        var shape = Shape2.Invoke(3, 4);
        Result<int, ErrorCode, Error<ErrorCode>> result = shape.Index(new[] { 1 });
        result.IsFailed.Should().BeTrue();
    }
}
