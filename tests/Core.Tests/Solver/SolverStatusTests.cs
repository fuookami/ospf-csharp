...
public class SolverStatusTests {
    ...
    [Fact]
    public void Infeasible_ErrCode_ShouldBeORModelInfeasible() => SolverStatus.Infeasible.ErrCode().Should().Be(ErrorCode.ORModelInfeasible);

    [Fact]
    public void Unbounded_ErrCode_ShouldBeORModelUnbounded() => SolverStatus.Unbounded.ErrCode().Should().Be(ErrorCode.ORModelUnbounded);

    [Fact]
    public void InfeasibleOrUnbounded_ErrCode_ShouldBeORModelInfeasibleOrUnbounded() => SolverStatus.InfeasibleOrUnbounded.ErrCode().Should().Be(ErrorCode.ORModelInfeasibleOrUnbounded);

    [Fact]
    public void SolvingException_ErrCode_ShouldBeOREngineSolvingException() => SolverStatus.SolvingException.ErrCode().Should().Be(ErrorCode.OREngineSolvingException);

    [Fact]
    public void Optimal_ErrCode_ShouldBeNull() => SolverStatus.Optimal.ErrCode().Should().BeNull();

    [Fact]
    public void Feasible_ErrCode_ShouldBeNull() => SolverStatus.Feasible.ErrCode().Should().BeNull();
}
