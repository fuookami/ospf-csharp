...
public class VariableTypeTests {
    ...
    public void Binary_HasCorrectProperties() {
        Binary type = Binary.Instance;
        type.Name.Should().Be("Binary");
        type.ShortName.Should().Be("bin");
        IVariableTypeKind kind = AsKind(type);
    ...
    public void BalancedTernary_HasCorrectProperties() {
        BalancedTernary type = BalancedTernary.Instance;
    ...
    public void Percentage_HasCorrectProperties() {
        Percentage type = Percentage.Instance;
    ...
    public void UInteger_HasCorrectProperties() {
        UInteger type = UInteger.Instance;
    ...
    public void Continuous_HasCorrectProperties() {
        Continuous type = Continuous.Instance;
    ...
    public void UContinuous_HasCorrectProperties() {
        UContinuous type = UContinuous.Instance;
    ...
    public void IsContinuousType_IsNotIntegerType() {
        IVariableTypeKind kind = AsKind(Continuous.Instance);
        kind.IsContinuousType.Should().BeTrue();
        kind.IsIntegerType.Should().BeFalse();
    }

    [Fact]
    public void IsNotBinaryIntegerType_Integer_True() => AsKind(Integer.Instance).IsNotBinaryIntegerType.Should().BeTrue();

    [Fact]
    public void IsNotBinaryIntegerType_Binary_False() => AsKind(Binary.Instance).IsNotBinaryIntegerType.Should().BeFalse();
}
