...
namespace Fuookami.Ospf.Math.Symbol.Expression.Tests
{
    ...
    public class ScalarExpressionTests
    {
        ...
        {
            var c = ScalarExpressionFactory.Constant(42);
            Assert.IsType<ScalarConstant<int>>(c);

            var r = ScalarExpressionFactory.Reference<int>("age");
            Assert.IsType<ScalarReference<int>>(r);

            var u = ScalarExpressionFactory.Unary(UnaryOperator.Negate, c);
            Assert.IsType<ScalarUnary<int>>(u);

            var b = ScalarExpressionFactory.Binary(BinaryOperator.Add, c, c);
            Assert.IsType<ScalarBinary<int>>(b);

            var f = ScalarExpressionFactory.Function<int>("abs", new[] { c });
    ...
    public class BooleanExpressionTests
    {
        ...
        [Fact]
        public void AndExpression_shouldRequireAtLeastOneOperand()
        {
            Assert.Throws<ArgumentException>(() => new AndExpression(Array.Empty<BooleanExpression>()));
        }

        [Fact]
        public void OrExpression_shouldRequireAtLeastOneOperand()
        {
            Assert.Throws<ArgumentException>(() => new OrExpression(Array.Empty<BooleanExpression>()));
        }
