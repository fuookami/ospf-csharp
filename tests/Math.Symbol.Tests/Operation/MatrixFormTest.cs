...
public class MatrixFormTest {
    ...
    public void LinearMatrixForm_RoundTrip() {
        ...
        var order = new List<ISymbol> { _x, _y };
        LinearMatrixForm<Flt64> form = p.ToMatrixForm(order, Flt64.Zero).Value;
        LinearPolynomial<Flt64> reconstructed = MatrixFormOps.LinearPolynomialFromMatrixForm(form, Flt64.Zero);
    ...
    public void Flt64LinearMatrixForm_RoundTrip() {
        ...
        var order = new List<ISymbol> { _x, _y };
        Result<Flt64LinearMatrixForm, ErrorCode, Error<ErrorCode>> result = p.ToFlt64MatrixForm(order);
        result.IsOk.Should().BeTrue();

        Flt64LinearMatrixForm form = result.Value;
        form.C.Should().Equal(3.0, 2.0);
        form.D.Should().Be(new Flt64(5.0));

        LinearPolynomial<Flt64> reconstructed = Flt64MatrixFormOps.Flt64LinearPolynomialFromMatrixForm(form);
    ...
    public void QuadraticPolynomial_ToMatrixForm() {
        ...
        var order = new List<ISymbol> { _x, _y };
        Result<QuadraticMatrixForm<Flt64>, ErrorCode, Error<ErrorCode>> result = p.ToMatrixForm(order, Flt64.Zero, c => {
            Flt64 half = c / new Flt64(2.0);
            return (half, half);
        });
