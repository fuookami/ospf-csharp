...
namespace Fuookami.Ospf.Framework.Solver
{
    ...
    public sealed class ParallelCombinatorialColumnGenerationSolver : IColumnGenerationSolver
    {
            ...
            SolvingStatusCallBack? solvingStatusCallBack = null)
        {
            ...
            return _solverFactories[0]().SolveMILPAsync(name, metaModel, amount, toLogModel, registrationStatusCallBack, solvingStatusCallBack);
        }
        ...
        {
            return _solverFactories[0]().SolveMILPAsAsync(name, metaModel, converter, toLogModel, registrationStatusCallBack, solvingStatusCallBack);
        }
        ...
        public Task<Result<IColumnGenerationSolver.LpResultOf<V>, ErrorCode, Error<ErrorCode>>> SolveLPAsAsync<V>(
            ...
            where V : struct, IRealNumber<V>, INumberField<V>
        {
            return _solverFactories[0]().SolveLPAsAsync(name, metaModel, converter, toLogModel, registrationStatusCallBack, solvingStatusCallBack);
        }
    ...
    public sealed class SerialCombinatorialColumnGenerationSolver : IColumnGenerationSolver
    {
        ...
        public async Task<Result<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>> SolveMILPAsync(
            ...
            SolvingStatusCallBack? solvingStatusCallBack = null)
        {
            foreach (var factory in _solverFactories)
            {
                try
                {
                    var result = await factory().SolveMILPAsync(name, metaModel, toLogModel, registrationStatusCallBack, solvingStatusCallBack);
                    if (result is Ok<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>) return result;
                }
                catch (Exception e)
                {
            ...
            SolvingStatusCallBack? solvingStatusCallBack = null)
        {
            foreach (var factory in _solverFactories)
            {
                try
                {
                    var result = await factory().SolveLPAsync(name, metaModel, toLogModel, registrationStatusCallBack, solvingStatusCallBack);
                    if (result is Ok<IColumnGenerationSolver.LpResult, ErrorCode, Error<ErrorCode>>) return result;
                }
                catch (Exception e)
                {
                    _logger.LogError(e, "serial CG solver threw");
