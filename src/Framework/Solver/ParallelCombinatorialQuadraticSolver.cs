...
namespace Fuookami.Ospf.Framework.Solver
{
    ...
    public sealed class ParallelCombinatorialQuadraticSolver : IQuadraticSolver
    {
        ...
        private static async Task<Result<T, ErrorCode, Error<ErrorCode>>> BestSuccessAsync<T>(IEnumerable<Task<Result<T, ErrorCode, Error<ErrorCode>>>> tasks) where T : class
        {
            ...
            if (success.Count == 0) return new Failed<T, ErrorCode, Error<ErrorCode>>(new Err<ErrorCode>(ErrorCode.SolverNotFound, "No solver succeeded"));
    ...
    public sealed class SerialCombinatorialQuadraticSolver : IQuadraticSolver
    {
            ...
            CancellationToken cancellationToken = default)
        {
            foreach (var factory in _solverFactories)
            {
                try
                {
                    var result = await factory().InvokeAsync(model, solvingStatusCallBack, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            foreach (var factory in _solverFactories)
            {
                try
                {
                    var result = await factory().InvokeAsync(model, solvingStatusCallBack, iisConfig, cancellationToken);
                    if (result is Ok<SolverOutput, ErrorCode, Error<ErrorCode>>) return result;
                }
                catch (Exception e) { _logger.LogError(e, "serial quadratic solver threw"); }
            }
