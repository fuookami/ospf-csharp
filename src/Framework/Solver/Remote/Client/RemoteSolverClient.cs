...
namespace Fuookami.Ospf.Framework.Solver.Remote.Client
{
    ...
    public sealed class RemoteSolverClient
    {
            ...
            bool exportCheckpointEachRound = true)
        {
            ...
            {
                rounds++;
                var slice = await _executionPort.AwaitSliceEndAsync(handle, quantum);
                ...
                {
                    var cp = await _executionPort.ExportCheckpointAsync(handle);
                ...
                {
                    var fetched = await _executionPort.FetchFinalResultAsync(handle);
