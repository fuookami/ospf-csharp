...
namespace Fuookami.Ospf.Framework.Model
{
    ...
    /// </summary>
    /// <param name="Limit">约束限制类型 / Constraint limit type.</param>
        ...
        where TSelf : AbstractShadowPriceMap<Args, TSelf>
    {
        ...
        public ShadowPrice? this[ShadowPriceKey key] => _map.TryGetValue(key, out var v) ? v : null;
        ...
        public void PutOrAdd(ShadowPrice price)
        {
            _map[price.Key] = new ShadowPrice(
                price.Key,
                (_map.TryGetValue(price.Key, out var cur) ? cur.Price : Flt64.Zero) + price.Price);
        }
        ...
        public void Shrink()
        {
            foreach (var key in _map.Where(kv => kv.Value.Price == Flt64.Zero).Select(kv => kv.Key).ToList())
            {
                _map.Remove(key);
            }
        }
    ...
    internal static class ShadowPriceHelpers
    {
            ...
            where TMap : AbstractShadowPriceMap<Args, TMap>
        {
            ...
            {
                var ret = pipeline.Refresh(shadowPriceMap, model, shadowPrices);
