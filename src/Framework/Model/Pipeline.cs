...
public interface ICGPipeline<Args, in MModel, TMap> : IPipeline<MModel>
    ...
    Try Refresh(TMap shadowPriceMap, MModel model, MetaDualSolution shadowPrices) {
        ...
        if (model is not IMetaModel<Flt64> metaModel) {
            return Results.Ok<Success>(Results.SuccessInstance);
        }

        var prices = new Dictionary<ShadowPriceKey, Flt64>();
        foreach (MathConstraint constraint in metaModel.ConstraintsOfGroup(this)) {
            if (constraint.Args is ShadowPriceKey key) {
                if (shadowPrices.Constraints.TryGetValue(constraint, out Flt64 price)) {
                    prices[key] = (prices.TryGetValue(key, out Flt64 p) ? p : Flt64.Zero) + price;
                }
            }
        }
        foreach ((ShadowPriceKey? key, Flt64 value) in prices) {
    ...
    static Try RefreshByKeyAsArgs<TMapT>(
        ...
        foreach (MathConstraint constraint in metaModel.ConstraintsOfGroup(pipeline)) {
            if (constraint.Args is not ShadowPriceKey key) {
                continue;
            }

            if (shadowPrices.Constraints.TryGetValue(constraint, out Flt64 price)) {
                thisShadowPrices[key] = (thisShadowPrices.TryGetValue(key, out Flt64 p) ? p : Flt64.Zero) + price;
            }
        }
        foreach ((ShadowPriceKey? key, Flt64 value) in thisShadowPrices) {
            shadowPriceMap.Put(new ShadowPrice(key, value));
        }
        return Results.Ok<Success>(Results.SuccessInstance);
    }
...
public interface IHAPipeline<M> : IPipeline<M> where M : class {
    ...
    Result<HAPipelineObj, ErrorCode, Error<ErrorCode>> Analyze(M model, IReadOnlyList<Flt64> solution) {
        Result<Flt64?, ErrorCode, Error<ErrorCode>> obj = Calculate(model, solution);
...
public static class PipelineListInvoker {
    ...
    public static Try Invoke<M>(IReadOnlyList<IPipeline<M>> pipelineList, M model) where M : class {
        foreach (IPipeline<M> pipeline in pipelineList) {
    ...
    public static Result<IReadOnlyList<HAPipelineObj>, ErrorCode, Error<ErrorCode>> InvokeHA<M>(
        ...
        foreach (IHAPipeline<M> pipeline in pipelineList) {
            Result<HAPipelineObj, ErrorCode, Error<ErrorCode>> ret = pipeline.Analyze(model, solution);
