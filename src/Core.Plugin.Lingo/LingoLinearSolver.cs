...
namespace Fuookami.Ospf.Core.Plugin.Lingo;

public sealed class LingoLinearSolver : ILinearSolver {
    public string Name => "lingo";
    public SolverConfig Config { get; }

    public LingoLinearSolver(SolverConfig? config = null) {
    ...
    public Task<Result<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>> InvokeAsync(
        ...
        => Task.FromResult<Result<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>>(
            new Failed<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>(
                new Err<ErrorCode>(ErrorCode.OREngineEnvironmentLost, "Lingo managed DLL not loaded at runtime")));
