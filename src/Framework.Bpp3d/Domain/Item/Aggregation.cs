...
namespace Fuookami.Ospf.Framework.Bpp3d.Domain.Item
{
    ...
    public sealed class ItemAggregation<V> : IItemAggregation<V> where V : struct, IFloatingNumber<V>
    {
        ...
        public IReadOnlyDictionary<Item<V>, ulong> RestItems =>
            Schemas.SelectMany(s => s.PatternedItems)
                ...
                .ToDictionary(
                    ...
                    g =>
                    {
                        var total = g.Aggregate(0L, (acc, p) => acc + (long)p.Amount.ToFlt64().ToDouble());
                        var used = _usedItems.TryGetValue(g.Key, out var u) ? u : 0UL;
        ...
        public void UseItems(IReadOnlyDictionary<Item<V>, ulong> items)
        {
            ...
            {
                _usedItems[item] = (_usedItems.TryGetValue(item, out var cur) ? cur : 0) + amount;
            }
        }
        ...
        {
            foreach (var (bin, amount) in bins)
            {
                _usedBins[bin] = (_usedBins.TryGetValue(bin, out var cur) ? cur : 0) + amount;
            }
        }
    }
}
