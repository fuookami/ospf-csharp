...
public abstract class AbstractBpp3dShadowPriceMap {
    ...
    public Result<Flt64, ErrorCode, Error<ErrorCode>> PriceOf(DemandShadowPriceKey key) {
        ...
        return _prices.TryGetValue(key, out Flt64 value)
            ? Results.Ok(value)
    ...
    public Result<Success, ErrorCode, Error<ErrorCode>> RegisterRange(
        ...
        foreach ((DemandShadowPriceKey? key, Flt64 price) in entries) {
            Result<Success, ErrorCode, Error<ErrorCode>> r = Register(key, price);
            if (r.IsFailed) {
                return r;
            }
        }
