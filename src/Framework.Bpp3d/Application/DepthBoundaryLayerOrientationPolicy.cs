...
public sealed record DepthBoundaryLayerOrientationPolicy(
    ...
    internal Result<Success, ErrorCode, Error<ErrorCode>> EnsureSatisfied(IReadOnlyList<Bin<BinLayer, FltX>> bins) {
        ...
        for (int i = 0; i < bins.Count; i++) {
            ...
            Result<Success, ErrorCode, Error<ErrorCode>> first = CheckBoundary(i, true, ordered.First());
            if (first.IsFailed) {
                return first;
            }

            Result<Success, ErrorCode, Error<ErrorCode>> last = CheckBoundary(i, false, ordered.Last());
            if (last.IsFailed) {
                return last;
            }
        }
    ...
    private Result<Success, ErrorCode, Error<ErrorCode>> CheckBoundary(int binIndex, bool isFirst, QuantityPlacement3<BinLayer, FltX> placement) {
        IReadOnlySet<Axis3>? allowedAxes = isFirst ? FirstLayerAllowedCylinderAxes : LastLayerAllowedCylinderAxes;
        IReadOnlySet<Orientation>? allowedOrientations = isFirst ? FirstLayerAllowedCuboidOrientations : LastLayerAllowedCuboidOrientations;
        if (allowedAxes is null && allowedOrientations is null) {
            return Results.Ok<Success>(Results.SuccessInstance);
        }

        string side = isFirst ? "first" : "last";

        foreach (QuantityPlacement3<Item, FltX> unit in placement.Unit.Units) {
