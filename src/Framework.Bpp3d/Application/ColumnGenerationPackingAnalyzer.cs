...
public sealed class ColumnGenerationPackingAnalyzer {
    ...
    public async Task<Result<Success, ErrorCode, Error<ErrorCode>>> AnalyzeAsync(ColumnGenerationState<FltX> state) {
        ...
        if (state.Bins.Count > 0) {
            ...
            foreach (BinLayer layer in state.Columns) {
                if (layer.Bin is null) {
                    continue;
                }

                QuantityPlacement3<BinLayer, FltX> placement = BinLayerHelpers.BinLayerPlacementOf(layer.Copy(), BinLayerHelpers.Point3FltX());
                binList.Add(BinLayerHelpers.LayerBinOf(layer.Bin, new[] { placement }));
            }
            bins = binList;
        }

        Result<PackingResult, ErrorCode, Error<ErrorCode>> packingResultRet = await _packer.InvokeAsync(bins);
        if (packingResultRet is not Ok<PackingResult, ErrorCode, Error<ErrorCode>> packingOk) {
            Error<ErrorCode> err = (packingResultRet as Failed<PackingResult, ErrorCode, Error<ErrorCode>>)?.Error
                ?? new Err<ErrorCode>(ErrorCode.Unknown);
            return new Failed<Success, ErrorCode, Error<ErrorCode>>(err);
        }

        Result<SchemaDTO, ErrorCode, Error<ErrorCode>> schemaRet = _rendererAdapter.ToSchema(packingResultRet.Value);
        if (schemaRet is not Ok<SchemaDTO, ErrorCode, Error<ErrorCode>> schemaOk) {
            Error<ErrorCode> sErr = (schemaRet as Failed<SchemaDTO, ErrorCode, Error<ErrorCode>>)?.Error
                ?? new Err<ErrorCode>(ErrorCode.Unknown);
