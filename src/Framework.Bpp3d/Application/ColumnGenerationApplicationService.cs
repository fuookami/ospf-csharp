...
public sealed class ColumnGenerationApplicationService {
    ...
    public async Task<Result<ColumnGenerationApplicationResponse, ErrorCode, Error<ErrorCode>>> SolveAsync(
        ...
        IReadOnlyList<(Item Item, ulong Amount)> resolvedItemDemands = request.ItemDemands;
        IReadOnlyList<Bpp3dDemandEntry<FltX>> entries = request.DemandEntries ?? BuildDemandEntries(resolvedItemDemands);
        ...
        IReadOnlyList<IBpp3dLayerGenerator<FltX>> generators = request.Generators?.Count > 0 ? request.Generators : DefaultGenerators;
        ...
        Result<ColumnGenerationResult<FltX>, ErrorCode, Error<ErrorCode>> solveResult = await algorithm.SolveAsync(
        ...
        if (solveResult is not Ok<ColumnGenerationResult<FltX>, ErrorCode, Error<ErrorCode>> ok) {
            Error<ErrorCode> err = (solveResult as Failed<ColumnGenerationResult<FltX>, ErrorCode, Error<ErrorCode>>)?.Error
...
internal sealed class LayerGenerationAdapter : IBpp3dLayerGenerator<FltX> {
    ...
    public async Task<IReadOnlyList<Bpp3dLayerGenerationResult<FltX>>> GenerateAsync(Bpp3dLayerGenerationRequest<FltX> request) {
        ...
        foreach (IBpp3dLayerGenerator<FltX> gen in _generators) {
