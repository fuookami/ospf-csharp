...
namespace Fuookami.Ospf.Framework.Bpp3d.Application
{
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
            ...
            ColumnGenerationConfig? config = null)
        {
            ...
            var columns = _deduplicateColumns(await _initialColumns());
            ...
            var prototypes = ContinuousRadiusSolverHelpers.PrototypesFromItems(items);
            ...
            while (iterations < config.IterationLimit)
            {
                ...
                {
                    var rmpRet = await _rmpSolver(state);
                    if (rmpRet is not Ok<ColumnGenerationLpResult<V>, ErrorCode, Error<ErrorCode>> ok)
                    {
                        var err = (rmpRet as Failed<ColumnGenerationLpResult<V>, ErrorCode, Error<ErrorCode>>)?.Error
                ...
                var request = _layerRequestBuilder is not null
                ...
                var candidates = await _layerGenerator.GenerateAsync(request);
                var accepted = await _filterByReducedCost(refreshState, candidates.ToList());
                ...
                if (_heartbeat is not null) await _heartbeat(heartbeatState);
                iterations++;
            }

            if (iterations >= config.IterationLimit) terminatedByIterationLimit = true;
            ...
            if (config.FinalMilpEnabled)
            {
                ...
                {
                    var finalRet = await _finalMilpSolver(finalState);
                    if (finalRet is not Ok<ColumnGenerationFinalResult<V>, ErrorCode, Error<ErrorCode>> finalOk)
                    {
                        var fErr = (finalRet as Failed<ColumnGenerationFinalResult<V>, ErrorCode, Error<ErrorCode>>)?.Error
                            ?? new Err<ErrorCode>(ErrorCode.Unknown);
                ...
                if (finalResult is not null)
                {
                    ...
                    var solverResults = ContinuousRadiusSolverHelpers.ExtractResults(finalResult.Info ?? new Dictionary<string, string>());
            ...
            {
                var ar = await _solutionAnalyzer(finalState);
