    ...
    )
    {
            ...
            IReadOnlyList<ICuttingPlanSliceForCanonicalKey> slices)
        {
            var grouped = new SortedDictionary<(string, string, string), UInt64>(
                ...
                {
                    var c = string.Compare(a.Item1, b.Item1, StringComparison.Ordinal);
                    if (c != 0) return c;
            ...
            {
                var key = (slice.ProductionType, slice.ProductionId ?? "", slice.WidthKey);
            ...
            IReadOnlyList<ICuttingPlanDemandContributionForCanonicalKey> contributions)
        {
            ...
            {
                var key = (contribution.ProductId, contribution.UnitKey);
