...
internal sealed class GenerationQuantityCache<V> where V : struct {
    ...
    public Quantity<V> RepeatWidth(Quantity<V> width, long times) {
        (Quantity<V> width, long times) key = (width, times);
        if (_repeatWidthCache.TryGetValue(key, out Quantity<V>? existing)) {
            _repeatWidthHits++;
            return existing;
        }
        _repeatWidthMisses++;
        Quantity<V> result = _repeatWidth(width, times);
    ...
    public long MaxRepeatCount(Quantity<V> width, Quantity<V> availableWidth) {
        (Quantity<V> width, Quantity<V> availableWidth) key = (width, availableWidth);
        if (_maxRepeatCountCache.TryGetValue(key, out long existing)) {
            _maxRepeatCountHits++;
            return existing;
        }
        _maxRepeatCountMisses++;
        long result = _maxRepeatCount(width, availableWidth);
