...
namespace Fuookami.Ospf.Framework.Csp1d.Domain.CuttingPlanGeneration.Service
{
    ...
    internal sealed class CostarFiller<V> where V : struct, IComparable<V>
    {
        ...
        {
            if (costars.Count == 0) return new[] { plan };
            if (restWidth is not { } rw) return new[] { plan };
            if (_isZero(rw) || !_isPositive(rw)) return new[] { plan };
            ...
            long maxAmount)
        {
            ...
            var costar = costars[costarIndex];
            foreach (var costarWidth in getCostarWidths(costar))
            {
                if (_isLessThan(remainingWidth, costarWidth)) continue;

                var amountLimit = global::System.Math.Min(ComputeMaxCostarAmount(costarWidth, remainingWidth), maxAmount);
                for (long amount = 1L; amount <= amountLimit; amount++)
                {
                    var totalCostarWidth = RepeatWidth(costarWidth, amount);
                    var newRemaining = _subtractQuantity(remainingWidth, totalCostarWidth);
        ...
        {
            if (_isLessThan(remainingWidth, costarWidth)) return 0L;
            long count = 0L;
