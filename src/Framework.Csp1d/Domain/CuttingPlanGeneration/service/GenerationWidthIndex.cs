...
internal sealed class GenerationWidthIndex<V> where V : struct, IComparable<V> {
    ...
    public GenerationWidthIndex<V> Filter(Func<GenerationProductWidthEntry<V>, bool> predicate) {
        ...
        foreach (GenerationProductWidthEntry<V> entry in _entries) {
            if (predicate(entry)) {
                filtered.Add(entry);
            }
        }
    ...
    public bool HasFittableFrom(int startIndex, Quantity<V> remainingWidth) {
        ...
        string unitKey = CanonicalUnitKey(remainingWidth.Unit);
        if (!_suffixMinWidthByUnit[startIndex].TryGetValue(unitKey, out Quantity<V>? minWidth)) {
            return false;
        }
    ...
    public PhysicalUnit? DemandUnitFor(string productId, Quantity<V> width) {
        (string productId, string, string) key = (productId, width.Value.ToString() ?? "", CanonicalUnitKey(width.Unit));
        return _demandUnitIndex.TryGetValue(key, out PhysicalUnit? unit) ? unit : null;
    }
    ...
    public static GenerationWidthIndex<V> FromDemands<TDemand>(
        ...
        foreach (TDemand? demand in demands) {
            string productId = getProductId(demand);
            PhysicalUnit demandUnit = getDemandUnit(demand);
            foreach (Quantity<V> width in getWidths(demand)) {
                (string productId, string, string, string) key = (productId, width.Value.ToString() ?? "", CanonicalUnitKey(width.Unit), CanonicalUnitKey(demandUnit));
                if (!seen.Add(key)) {
                    continue;
                }
    ...
    private static GenerationWidthIndex<V> FromEntries(IReadOnlyList<GenerationProductWidthEntry<V>> entries) {
        IReadOnlyList<Dictionary<string, Quantity<V>>> suffixMinWidthByUnit = BuildSuffixMinWidthByUnit(entries);
        Dictionary<(string, string, string), PhysicalUnit> demandUnitIndex = BuildDemandUnitIndex(entries);
    ...
    private static IReadOnlyList<Dictionary<string, Quantity<V>>> BuildSuffixMinWidthByUnit(
        ...
        for (int i = entries.Count - 1; i >= 0; i--) {
            GenerationProductWidthEntry<V> entry = entries[i];
            string unitKey = CanonicalUnitKey(entry.Width.Unit);
            if (!current.TryGetValue(unitKey, out Quantity<V>? existing) || entry.Width.Value.CompareTo(existing.Value) < 0) {
    ...
    private static Dictionary<(string, string, string), PhysicalUnit> BuildDemandUnitIndex(
        ...
        foreach (GenerationProductWidthEntry<V> entry in entries) {
            (string ProductId, string, string) key = (entry.ProductId, entry.Width.Value.ToString() ?? "", CanonicalUnitKey(entry.Width.Unit));
            index.TryAdd(key, entry.DemandUnit);
        }
        return index;
    }

    internal static string CanonicalUnitKey(PhysicalUnit unit) => unit.Symbol ?? unit.Name ?? unit.ToString();
}
