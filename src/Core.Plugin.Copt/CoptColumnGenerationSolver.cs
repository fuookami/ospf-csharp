...
/// <summary>
/// COPT 列生成求解器实现（桩）。
/// COPT column generation solver implementation (stub).
///
/// 使用 COPT 求解器实现列生成策略，支持 MILP 求解、多解求解和 LP 松弛求解。
/// 运行时需加载 COPT 托管 DLL，否则返回 OREngineEnvironmentLost。
/// Implements column generation strategy using COPT solver, supporting MILP solving,
/// multi-solution solving, and LP relaxation solving.
/// Requires COPT managed DLL at runtime; returns OREngineEnvironmentLost otherwise.
/// </summary>
public sealed class CoptColumnGenerationSolver : Framework.Solver.IColumnGenerationSolver {
    ...
    /// <summary>
    /// 创建 COPT 列生成求解器 / Create COPT column generation solver.
    /// </summary>
    /// <param name="config">求解器配置 / Solver configuration.</param>
    public CoptColumnGenerationSolver(SolverConfig? config = null) {
    ...
    /// <summary>
    /// 求解 LP 松弛问题 / Solve LP relaxation problem.
    ///
    /// COPT 桩实现不支持对偶解提取，返回空对偶解字典。
    /// COPT stub does not support dual solution extraction; returns empty dual dictionary.
    /// </summary>
    /// <param name="name">模型名称 / Model name.</param>
    /// <param name="metaModel">线性元模型 / Linear meta model.</param>
    /// <param name="toLogModel">是否记录模型日志 / Whether to log model.</param>
