...
namespace Fuookami.Ospf.Framework.Plugin.Persistence.SQLite
{
    /// <summary>
    /// SQLite EF Core 仓储基类 / SQLite EF Core repository base class.
    ///
    /// 提供基于 EF Core + Microsoft.Data.Sqlite 的通用仓储实现。
    /// Provides a generic repository implementation backed by EF Core with SQLite.
    /// </summary>
    ...
    public abstract class SqliteRepository<T> : IRepository<T> where T : class
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                return new Failed<T?, ErrorCode, Error<ErrorCode>>(
                    ErrorCode.DataNotFound, $"SQLite FindById failed: {ex.Message}");
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                var results = await Set.ToListAsync(cancellationToken);
                return Results.Ok<IReadOnlyList<T>>(results);
            }
            catch (Exception ex)
            {
                return new Failed<IReadOnlyList<T>, ErrorCode, Error<ErrorCode>>(
                    ErrorCode.ApplicationError, $"SQLite FindAll failed: {ex.Message}");
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                return new Failed<Success, ErrorCode, Error<ErrorCode>>(
                    ErrorCode.ApplicationFailed, $"SQLite Insert failed: {ex.Message}");
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return new Failed<Success, ErrorCode, Error<ErrorCode>>(
                    ErrorCode.ApplicationFailed, $"SQLite InsertMany failed: {ex.Message}");
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                return new Failed<long, ErrorCode, Error<ErrorCode>>(
                    ErrorCode.ApplicationFailed, $"SQLite Update failed: {ex.Message}");
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return new Failed<Success, ErrorCode, Error<ErrorCode>>(
                    ErrorCode.ApplicationFailed, $"SQLite Delete failed: {ex.Message}");
            }
        }
    }
}
