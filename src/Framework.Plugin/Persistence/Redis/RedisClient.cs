...
public sealed class RedisClient : IDisposable {
    ...
    /// </summary>
    public void Set(string name, string value, TimeSpan? ex = null) => _database.StringSet(name, value, ex);
    ...
    public List<string>? GetList(string name) {
        RedisValue[] values = _database.ListRange(name);
    ...
    public HashSet<string>? GetSet(string name) {
        RedisValue[] values = _database.SetMembers(name);
    ...
    public Dictionary<string, string>? GetMap(string name) {
        HashEntry[] entries = _database.HashGetAll(name);
...
public static class Redis {
    ...
    public static RedisClient? Init(RedisConfig config) {
        lock (_lock) {
            if (_clients.TryGetValue(config.Key, out IConnectionMultiplexer? existing)) {
    ...
    public static RedisClient? Get(RedisClientKey key) {
        lock (_lock) {
            return _clients.TryGetValue(key, out IConnectionMultiplexer? multiplexer)
    ...
    public static RedisClient? GetByName(string name, int? database = null) {
        lock (_lock) {
            KeyValuePair<RedisClientKey, IConnectionMultiplexer> match = _clients.FirstOrDefault(kv =>
