...
/// <summary>
/// Gurobi 11 列生成求解器（桩实现）。
/// Gurobi 11 column generation solver (structural stub).
///
...
/// </summary>
public sealed class Gurobi11ColumnGenerationSolver : Framework.Solver.IColumnGenerationSolver {
    ...
    /// <summary>
    /// 创建 Gurobi 11 列生成求解器 / Create Gurobi 11 column generation solver.
    /// </summary>
    /// <param name="config">求解器配置 / Solver configuration.</param>
    public Gurobi11ColumnGenerationSolver(SolverConfig? config = null) {
        Config = config ?? new VendorSolverConfig();
    }

    // ===== IColumnGenerationSolver: MILP =====
    ...
    // ===== IColumnGenerationSolver: LP =====

    /// <summary>
    /// 求解 LP 松弛问题（桩实现，无法提取对偶解）。
    /// Solve LP relaxation problem (stub — dual extraction not available).
    /// </summary>
    /// <param name="name">模型名称 / Model name.</param>
    /// <param name="metaModel">线性元模型 / Linear meta model.</param>
    /// <param name="toLogModel">是否记录模型日志 / Whether to log model.</param>
    /// <param name="registrationStatusCallBack">注册状态回调 / Registration status callback.</param>
    /// <param name="solvingStatusCallBack">求解状态回调 / Solving status callback.</param>
    /// <returns>LP 求解结果（空对偶解字典）/ LP solving result (empty dual solution dictionary).</returns>
