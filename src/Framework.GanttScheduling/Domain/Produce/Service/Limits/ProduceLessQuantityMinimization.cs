...
/// <summary>
/// 生产下限数量最小化 / Produce less quantity minimization
/// </summary>
/// <typeparam name="P">产品类型 / Product type</typeparam>
public sealed class ProduceLessQuantityMinimization<P> : IPipeline<object>
    where P : IMaterial {
    ...
    /// <summary>
    /// 生产下限数量最小化构造 / Produce less quantity minimization constructor
    /// </summary>
    ...
    public ProduceLessQuantityMinimization(
        IReadOnlyList<(P Product, MaterialDemand? Demand)> products,
        IProduce produce,
        Func<P, Flt64>? threshold = null,
        Func<P, Flt64>? coefficient = null,
        string name = "produce_less_quantity_minimization") {
        _products = produce.LessEnabled
            ? products.Where(p => p.Demand?.LessEnabled == true).ToList()
