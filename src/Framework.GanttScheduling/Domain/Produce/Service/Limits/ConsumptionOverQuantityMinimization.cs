...
public sealed class ConsumptionOverQuantityMinimization<C> : IPipeline<object>
    where C : IMaterial {
    ...
    /// <summary>
    /// 消费超限数量最小化构造 / Consumption over quantity minimization constructor
    /// </summary>
    ...
    public ConsumptionOverQuantityMinimization(
        ...
        string name = "consumption_over_quantity_minimization") {
        _materials = consumption.OverEnabled
            ? materials.Where(m => m.Reserves?.OverEnabled == true).ToList()
