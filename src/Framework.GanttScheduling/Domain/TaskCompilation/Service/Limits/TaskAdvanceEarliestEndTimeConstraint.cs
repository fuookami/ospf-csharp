...
public sealed class TaskAdvanceEarliestEndTimeShadowPriceKey<E, A> : ShadowPriceKey
    where E : Executor
    where A : IAssignmentPolicy<E> {
    public TaskAdvanceEarliestEndTimeShadowPriceKey(IAbstractTask<E, A> task) : base(typeof(TaskAdvanceEarliestEndTimeShadowPriceKey<E, A>)) {
        Task = task;
    }
    public IAbstractTask<E, A> Task { get; }
    public override bool Equals(object? obj) => obj is TaskAdvanceEarliestEndTimeShadowPriceKey<E, A> other && Equals(Task, other.Task);
...
/// <typeparam name="E">执行器类型 / Executor type</typeparam>
/// <typeparam name="A">分配策略类型 / Assignment policy type</typeparam>
public class TaskAdvanceEarliestEndTimeConstraint<E, A>
    ...
    /// <summary>
    /// 任务提前最早结束时间约束构造 / Task advance earliest end time constraint constructor
    /// </summary>
    ...
    public TaskAdvanceEarliestEndTimeConstraint(
        TimeWindow<Flt64> timeWindow,
        IReadOnlyList<IAbstractTask<E, A>> tasks,
        TaskTime taskTime,
        string name = "task_advance_earliest_end_time") {
    ...
    public ShadowPriceExtractor<IGanttSchedulingShadowPriceArguments<E, A>, GanttSchedulingShadowPriceMap<E, A>>? Extractor()
        => (map, args) => {
            if (args.Task != null) {
                var key = new TaskAdvanceEarliestEndTimeShadowPriceKey<E, A>(args.Task);
