...
/// <summary>
/// 任务超过最大延迟时间影子价格键 / Task over-max delay shadow price key
/// </summary>
/// <typeparam name="E">执行器类型 / Executor type</typeparam>
/// <typeparam name="A">分配策略类型 / Assignment policy type</typeparam>
public sealed class TaskOverMaxDelayShadowPriceKey<E, A> : ShadowPriceKey
    where E : Executor
    where A : IAssignmentPolicy<E> {
    public TaskOverMaxDelayShadowPriceKey(IAbstractTask<E, A> task) : base(typeof(TaskOverMaxDelayShadowPriceKey<E, A>)) {
        Task = task;
    }
    public IAbstractTask<E, A> Task { get; }
    public override bool Equals(object? obj) => obj is TaskOverMaxDelayShadowPriceKey<E, A> other && Equals(Task, other.Task);
    public override int GetHashCode() => Task?.GetHashCode() ?? 0;
}

/// <summary>
/// 任务超过最大延迟时间约束 / Task over-max delay time constraint
/// </summary>
/// <typeparam name="E">执行器类型 / Executor type</typeparam>
/// <typeparam name="A">分配策略类型 / Assignment policy type</typeparam>
public class TaskOverMaxDelayTimeConstraint<E, A>
    ...
    /// <summary>
    /// 任务超过最大延迟时间约束构造 / Task over-max delay time constraint constructor
    /// </summary>
    ...
    public TaskOverMaxDelayTimeConstraint(
        TimeWindow<Flt64> timeWindow,
        IReadOnlyList<IAbstractTask<E, A>> tasks,
        TaskTime taskTime,
        string name = "task_over_max_delay_time") {
    ...
    public ShadowPriceExtractor<IGanttSchedulingShadowPriceArguments<E, A>, GanttSchedulingShadowPriceMap<E, A>>? Extractor()
        => (map, args) => {
            if (args.Task != null) {
                var key = new TaskOverMaxDelayShadowPriceKey<E, A>(args.Task);
