...
namespace Fuookami.Ospf.Framework.GanttScheduling.Application.Model.Task;
/// <summary>
/// 任务迭代器 / Task iterator
///
/// 跟踪基于任务的分支定价算法的迭代状态，管理收敛检测、LP/IP 目标跟踪、上下界和最优率计算。
/// Tracks iteration state of task-based branch-and-price algorithm, managing convergence detection,
...
public class TaskIteration<IT, E, A>
    ...
    /// <summary>
    /// 任务迭代器构造 / Task iteration constructor
    /// </summary>
    ...
    public void RefreshLowerBound(
        ...
        foreach (IT task in newTasks) {
            Flt64 thisReducedCost = reducedCost(task);
            if (task.Executor is not null) {
                if (bestReducedCost.TryGetValue(task.Executor, out Flt64 existing)) {
                    bestReducedCost[task.Executor] = thisReducedCost < existing ? thisReducedCost : existing;
                }
                else {
                    bestReducedCost[task.Executor] = thisReducedCost;
                }
            }
        }
