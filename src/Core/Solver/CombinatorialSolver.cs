...
namespace Fuookami.Ospf.Core.Solver
{
    ...
    public sealed class ParallelCombinatorialQuadraticSolver : IAbstractQuadraticSolver
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var solver = f();
                var ret = await solver.InvokeAsync(model, solvingStatusCallBack, cts.Token);
                return (solver, ret);
            }, cts.Token)).ToList();
            ...
            while (tasks.Count > 0)
            {
                ...
                var (solver, ret) = await completed;
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var solver = f();
                var ret = await solver.InvokeAsync(model, solvingStatusCallBack, cancellationToken);
                return (solver, ret);
            ...
            var isMax = model.Objective.Category == ObjectCategory.Maximum;
            var best = isMax
    ...
    public sealed class ParallelCombinatorialColumnGenerationSolver : IColumnGenerationSolver
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var solver = f();
                var ret = await solver.SolveMILPAsync(name, metaModel, toLogModel, registrationStatusCallBack, solvingStatusCallBack, cts.Token);
                return (solver, ret);
            }, cts.Token)).ToList();
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var solver = f();
                var ret = await solver.SolveMILPAsync(name, metaModel, toLogModel, registrationStatusCallBack, solvingStatusCallBack, cancellationToken);
                return (solver, ret);
            }, cancellationToken)).ToList();
            ...
            var best = oks.MinBy(o => o.Obj.ToDouble());
            return Results.Ok(best!);
        }
    }
}
