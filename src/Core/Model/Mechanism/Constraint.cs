...
namespace Fuookami.Ospf.Core.Model.Mechanism
{
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
        ...
        {
            var constraintRelation = relation.ConstraintRelation();
            var flattenData = relation.FlattenData;
            ...
            var flt64Monomials = flattenData.Monomials
                ...
                .ToList();
            var cells = ConstraintCellFactory.CreateLinearCells(flt64Monomials, tokens, converter);
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
            ...
            (IIntermediateSymbol Symbol, bool Direction)? from = null)
        {
            ...
            var flt64Monomials = flattenData.Monomials
                ...
                .ToList();
            var cells = ConstraintCellFactory.CreateQuadraticCells(flt64Monomials, tokens, converter);
            V rhs = default(V).Minus(flattenData.Constant);
    ...
    internal static class ConstraintCellFactory
    {
            ...
            where V : struct, IRealNumber<V>, INumberField<V>
        {
            ...
            {
                if (mono.Symbol is not IVariableItem variable) continue;
                var token = tokens.Find(variable);
            ...
            where V : struct, IRealNumber<V>, INumberField<V>
        {
            ...
            foreach (var mono in monomials)
            {
                if (mono.Symbol1 is not IVariableItem variable1) continue;
                var token1 = tokens.Find(variable1);
                if (token1 is null) continue;
                Token<V>? token2 = null;
                if (mono.Symbol2 is IVariableItem variable2)
                {
                    token2 = tokens.Find(variable2);
                    if (token2 is null) continue;
                }
