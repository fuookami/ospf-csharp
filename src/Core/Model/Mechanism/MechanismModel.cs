...
namespace Fuookami.Ospf.Core.Model.Mechanism
{
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
        ...
        public new SingleObject ObjectFunction { get; }
            ...
            MechanismModelDumpingStatusCallBack? dumpingStatusCallBack = null)
        {
            ...
            foreach (var mc in metaModel.RelationConstraints)
            {
                var flattenResult = mc.FlattenData();
                ...
                var flattenData = ((Ok<SymbolFlatten.LinearFlattenData<V>, ErrorCode, Error<ErrorCode>>)flattenResult).Value;
                var relation = new LinearRelationImpl<V>(flattenData, mc.Sign, mc.Name, mc.DisplayName);
                var constraintResult = LinearConstraintImpl<V>.Create(relation, tokens, metaModel.Converter,
                    mc.Lazy, mc.Name, mc);
            ...
            foreach (var source in metaModel.FlattenSubObjects)
            {
                var cells = ConstraintCellFactory.CreateLinearCells(
            ...
            foreach (var source in metaModel.MetaSubObjects)
            {
        ...
        public Try AddConstraint(
            ...
            (IIntermediateSymbol Symbol, bool Direction)? from = null)
        {
            var flattenResult = relation.ToLinearFlattenData<V>();
            if (flattenResult is Failed<SymbolFlatten.LinearFlattenData<V>, ErrorCode, Error<ErrorCode>> f)
            ...
            var result = LinearConstraintImpl<V>.Create(rel, Tokens, Parent.Converter, false, name ?? "", from: from);
        ...
        public Try AddConstraint(
            ...
            IIntermediateSymbol? from = null)
        {
            return AddConstraint(relation, name, from is not null ? (from, false) : null);
        }
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
            ...
            MechanismModelDumpingStatusCallBack? dumpingStatusCallBack = null)
        {
            ...
            foreach (var mc in metaModel.RelationConstraints)
            {
                ...
                var result = QuadraticConstraintImpl<V>.Create(relation, tokens, metaModel.Converter,
                    mc.Lazy, mc.Name, mc);
            ...
            (IIntermediateSymbol Symbol, bool Direction)? from = null)
        {
            ...
            var result = QuadraticConstraintImpl<V>.Create(rel, Tokens, Parent.Converter, false, name ?? "", from: from);
            if (result is Failed<QuadraticConstraintImpl<V>, ErrorCode, Error<ErrorCode>> cf)
                return Results.Failed<Success>(cf.Error);
    ...
    internal static class QuadraticFlattenExtensions
    {
            ...
            where V : struct, IRealNumber<V>, INumberField<V>
        {
            ...
            foreach (var mono in inequality.Lhs.Monomials)
            {
                var key = (mono.Symbol1, mono.Symbol2);
                if (merged.TryGetValue(key, out var existing))
                    merged[key] = new QuadraticMonomial<V>(existing.Coefficient.Plus(mono.Coefficient), mono.Symbol1, mono.Symbol2);
                else
                    merged[key] = mono;
            }
            foreach (var mono in inequality.Rhs.Monomials)
            {
                var key = (mono.Symbol1, mono.Symbol2);
                if (merged.TryGetValue(key, out var existing))
    ...
    internal static class LinearFlattenExtensions
    {
            ...
            where V : struct, IRealNumber<V>, INumberField<V>
        {
            ...
            foreach (var mono in inequality.Lhs.Monomials)
            {
                ...
                if (merged.TryGetValue(vi, out var existing))
