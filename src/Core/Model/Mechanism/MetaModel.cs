...
namespace Fuookami.Ospf.Core.Model.Mechanism
{
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
        ...
        public V? Evaluate(bool zeroIfNone = false)
        {
            return Evaluate(Parent.Tokens, zeroIfNone);
        }
        ...
        {
            V? result = null;
            foreach (var m in Polynomial.Monomials)
            {
                if (m.Symbol is not IVariableItem variable) return zeroIfNone ? default(V?) : null;
                var token = tokenTable.Find(variable);
                if (token is null || token.Result is null) return zeroIfNone ? default(V?) : null;
                var term = m.Coefficient.Times(token.Result.Value);
        ...
        public V? Evaluate(IReadOnlyList<V> results, bool zeroIfNone = false)
        {
            return Evaluate(results, Parent.Tokens, zeroIfNone);
        }
        ...
        public V? Evaluate(IReadOnlyList<V> results, Fuookami.Ospf.Core.Token.IAbstractTokenTable<V> tokenTable, bool zeroIfNone = false)
        {
            ...
            {
                if (m.Symbol is not IVariableItem variable) return zeroIfNone ? default(V?) : null;
                var idx = tokenTable.IndexOf(variable);
                if (idx is null) return zeroIfNone ? default(V?) : null;
                var term = m.Coefficient.Times(results[idx.Value]);
                result = result is null ? term : result.Value.Plus(term);
            }
            return result ?? Polynomial.Constant;
        }
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
        ...
        public Range? IndicesOfConstraintGroup(IMetaConstraintGroup group)
        {
            ...
            return _constraintGroupIndexMap.TryGetValue(group, out var range) ? range : null;
        }

        public IReadOnlyList<MathConstraint> ConstraintsOfGroup(IMetaConstraintGroup group)
        {
            var indices = IndicesOfConstraintGroup(group);
            if (indices is null) return Array.Empty<MathConstraint>();
            var (offset, length) = indices.Value.GetOffsetAndLength(Constraints.Count);
        ...
        {
            if (force) Tokens.TokenList.ClearSolution();
            Tokens.Flush();
        }
