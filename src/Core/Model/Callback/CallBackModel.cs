...
public sealed class FunctionalCallBackModelPolicy<V> : ICallBackModelPolicy<V>
    ...
    public FunctionalCallBackModelPolicy(
        ...
        Comparator = (lhs, rhs) => {
            if (_objectiveComparator is null) {
                return new Order.Equal();
            }

            if (_objectiveComparator(lhs, rhs)) {
                return new Order.Less();
            }

            if (_objectiveComparator(rhs, lhs)) {
    ...
    public IReadOnlyList<IReadOnlyList<V>> InitialSolutions(ulong initialSolutionAmount, ulong variableAmount) {
        if (_initialSolutionGenerator is null) {
            return Array.Empty<IReadOnlyList<V>>();
        }
...
public sealed class CallBackModel<V> : ICallBackModelInterface<V>
    ...
    V IAbstractCallBackModelInterface<V, V, V>.Objective(IReadOnlyList<V> solution) {
        ...
        foreach ((Func<IReadOnlyList<V>, V>? extractor, string _) in _objectiveFunctions) {
            V obj = extractor(solution);
    ...
    public bool? ConstraintSatisfied(IReadOnlyList<V> solution) {
        foreach (Token<V> token in Tokens.Tokens) {
            int? index = Tokens.IndexOf(token);
            if (index is null || index.Value >= solution.Count) {
                return false;
            }
        }
        foreach ((Func<IReadOnlyList<V>, bool?>? extractor, string _) in _constraints) {
            bool? result = extractor(solution);
            if (result == false) {
                return false;
            }
...
public sealed class MultiObjectCallBackModel<V> : IMultiObjectiveModelInterface<V>
    ...
    public List<V> DefaultObjective {
        get {
            V inf = ObjectCategory == ObjectCategory.Minimum ? NegativeInfinity() : Infinity();
    ...
    List<V> IAbstractCallBackModelInterface<List<(MultiObjectLocation<V> Location, V Value)>, List<V>, V>.ObjectiveValue(
        ...
        foreach ((MultiObjectLocation<V>? location, V objective) in obj) {
            int index = 0; foreach (MultiObjectLocation<V> loc in ObjectiveLocation) { if (loc.Priority == location.Priority) { break; } index++; }
            if (index < 0 || index >= ObjectiveLocation.Count) {
                continue;
            }
    ...
    List<V>? IAbstractCallBackModelInterface<List<(MultiObjectLocation<V> Location, V Value)>, List<V>, V>.Objective(
        ...
        foreach ((Func<IReadOnlyList<V>, List<(MultiObjectLocation<V> Location, V Value)>?>? extractor, string _) in _objectiveFunctions) {
            List<(MultiObjectLocation<V> Location, V Value)>? obj = extractor(solution);
            if (obj is null) {
                return null;
            }
    ...
    Order? IAbstractCallBackModelInterface<List<(MultiObjectLocation<V> Location, V Value)>, List<V>, V>.CompareObjective(
        ...
        if (lhs is not null && rhs is not null) {
            int size = System.Math.Min(lhs.Count, rhs.Count);
            for (int i = 0; i < size; i++) {
                Flt64 l = _converter.FromValue(lhs[i]);
                Flt64 r = _converter.FromValue(rhs[i]);
                if (l == r) {
                    continue;
                }
