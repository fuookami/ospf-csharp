...
internal static class FlattenUtility {
    ...
    public static LinearFlattenData<Flt64> MergeLinearMonomials(
        ...
        foreach (LinearMonomial<Flt64> m in monomials) {
            if (m.Symbol is not IVariableItem vi) {
                continue;
            }

            if (m.Coefficient != Flt64.Zero) {
                merged[vi] = (merged.TryGetValue(vi, out Flt64 existing) ? existing : Flt64.Zero) + m.Coefficient;
            }
        }
        var normalized = new List<LinearMonomial<Flt64>>(merged.Count);
        foreach ((IVariableItem? variable, Flt64 coefficient) in merged) {
    ...
    public static LinearFlattenData<Flt64> MergeLinearFlattenDataFlt64(
        ...
        var merged = new Dictionary<IVariableItem, Flt64>();
        Flt64 totalConstant = initialConstant;
        foreach (LinearFlattenData<Flt64> fd in flattenDataList) {
            totalConstant += fd.Constant;
            foreach (LinearMonomial<Flt64> m in fd.Monomials) {
                if (m.Symbol is not IVariableItem vi) {
                    continue;
                }
    ...
    public static QuadraticFlattenData<Flt64> MergeQuadraticMonomials(
        ...
        foreach (QuadraticMonomial<Flt64> m in monomials) {
            if (m.Symbol1 is not IVariableItem v1) {
                continue;
            }

            var v2 = m.Symbol2 as IVariableItem;
            (IVariableItem, IVariableItem?) key = v2 is not null
                ? (v1.Identifier < v2.Identifier ? (v1, (IVariableItem?)v2) : (v2, (IVariableItem?)v1))
                : (v1, (IVariableItem?)null);
            if (m.Coefficient != Flt64.Zero) {
                merged[key] = (merged.TryGetValue(key, out Flt64 existing) ? existing : Flt64.Zero) + m.Coefficient;
            }
        }
        var normalized = new List<QuadraticMonomial<Flt64>>(merged.Count);
        foreach (((IVariableItem, IVariableItem?) key, Flt64 coefficient) in merged) {
            if (coefficient != Flt64.Zero) {
    ...
    public static QuadraticFlattenData<Flt64> MergeQuadraticFlattenDataFlt64(
        ...
        var merged = new Dictionary<(IVariableItem, IVariableItem?), Flt64>();
        Flt64 totalConstant = initialConstant;
        foreach (QuadraticFlattenData<Flt64> fd in flattenDataList) {
            totalConstant += fd.Constant;
            foreach (QuadraticMonomial<Flt64> m in fd.Monomials) {
                if (m.Symbol1 is not IVariableItem v1) {
                    continue;
                }
    ...
    public static QuadraticFlattenData<Flt64> MultiplyLinear(
        ...
        foreach (LinearMonomial<Flt64> m1 in lhs.Monomials) {
            if (m1.Symbol is not IVariableItem v1) {
                continue;
            }

            foreach (LinearMonomial<Flt64> m2 in rhs.Monomials) {
                if (m2.Symbol is not IVariableItem v2) {
                    continue;
                }
    ...
    public static LinearFlattenData<Flt64> NormalizeLinear(LinearFlattenData<Flt64> data) {
        ...
        foreach (LinearMonomial<Flt64> m in data.Monomials) {
            if (m.Coefficient != Flt64.Zero) {
                filtered.Add(m);
            }
        }
    ...
    public static QuadraticFlattenData<Flt64> NormalizeQuadratic(QuadraticFlattenData<Flt64> data) {
        ...
        foreach (QuadraticMonomial<Flt64> m in data.Monomials) {
            if (m.Coefficient != Flt64.Zero) {
                filtered.Add(m);
            }
        }
