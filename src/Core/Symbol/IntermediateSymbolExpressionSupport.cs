    ...
    public sealed class LinearExpressionSymbol : ILinearIntermediateSymbol<Flt64>
    {
        ...
        public IReadOnlySet<IIntermediateSymbol> Dependencies
        {
            get
            {
                ...
                foreach (var m in _utilsPolynomial.Monomials)
                {
        ...
        public Flt64? Prepare(IReadOnlyDictionary<ISymbol, Flt64>? values, IAbstractTokenTable<Flt64> tokenTable, IFlt64ValueConverter<Flt64> converter)
        {
            ...
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var symVal = EvaluateSymbol(m.Symbol, values, tokenTable, false);
                if (symVal is null) return null;
        ...
        {
            var result = Prepare(values, tokenTable, converter);
            if (result is { } v)
        ...
        public Flt64? Evaluate(IAbstractTokenTable<Flt64> tokenTable, IFlt64ValueConverter<Flt64> converter, bool zeroIfNone = false)
        {
            ...
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var symVal = EvaluateSymbol(m.Symbol, null, tokenTable, zeroIfNone);
                if (symVal is null && !zeroIfNone) return null;
        ...
        {
            if (values.TryGetValue(this, out var selfVal)) return selfVal;
            var ret = _utilsPolynomial.Constant;
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var symVal = EvaluateSymbol(m.Symbol, values, tokenTable, zeroIfNone);
                if (symVal is null && !zeroIfNone) return null;
                ret += m.Coefficient * (symVal ?? Flt64.Zero);
            }
            return ret;
        ...
        public string ToRawString(UInt64 unfold)
        {
            ...
            {
                var parts = _utilsPolynomial.Monomials
                    .Where(m => m.Coefficient != Flt64.Zero)
                    .Select(m =>
                    {
                        var symStr = m.Symbol switch
                        {
                            IIntermediateSymbol ims => ims.ToRawString(unfold - UInt64.One),
                            ISymbol s => s.Name,
                            _ => "?"
                        };
                        if (m.Coefficient == Flt64.One) return symStr;
                        if (m.Coefficient == -Flt64.One) return $"-{symStr}";
        ...
        {
            if (values is not null && values.TryGetValue(symbol, out var v)) return v;
            if (tokenTable is null) return zeroIfNone ? Flt64.Zero : null;
    ...
    public sealed class QuadraticExpressionSymbol : IQuadraticIntermediateSymbol<Flt64>
    {
        ...
        public IReadOnlySet<IIntermediateSymbol> Dependencies
        {
            get
            {
                ...
                foreach (var m in _utilsPolynomial.Monomials)
                {
                    if (m.Symbol1 is IIntermediateSymbol s1) deps.Add(s1);
                    if (m.Symbol2 is IIntermediateSymbol s2) deps.Add(s2);
                }
                return deps;
            }
        }
        ...
        public Flt64? Prepare(IReadOnlyDictionary<ISymbol, Flt64>? values, IAbstractTokenTable<Flt64> tokenTable, IFlt64ValueConverter<Flt64> converter)
        {
            ...
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var s1Val = EvaluateSymbol(m.Symbol1, values, tokenTable, false);
                if (s1Val is null) return null;
                var termVal = m.Symbol2 is not null
                    ? (EvaluateSymbol(m.Symbol2!, values, tokenTable, false) is { } s2 ? s1Val.Value * s2 : (Flt64?)null)
                    : s1Val;
                if (termVal is null) return null;
        ...
        public Flt64? Evaluate(IAbstractTokenTable<Flt64> tokenTable, IFlt64ValueConverter<Flt64> converter, bool zeroIfNone = false)
        {
            ...
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var s1Val = EvaluateSymbol(m.Symbol1, null, tokenTable, zeroIfNone);
                if (s1Val is null && !zeroIfNone) return null;
                Flt64 termVal;
                if (m.Symbol2 is not null)
                {
                    var s2Val = EvaluateSymbol(m.Symbol2!, null, tokenTable, zeroIfNone);
                    if (s2Val is null && !zeroIfNone) return null;
                    termVal = (s1Val ?? Flt64.Zero) * (s2Val ?? Flt64.Zero);
                }
                else termVal = s1Val ?? Flt64.Zero;
        ...
        public Flt64? Evaluate(IReadOnlyList<Flt64> results, IAbstractTokenTable<Flt64> tokenTable, IFlt64ValueConverter<Flt64> converter, bool zeroIfNone = false)
        {
            ...
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var s1Val = EvaluateSymbolFromResults(m.Symbol1, results, tokenTable, zeroIfNone);
                if (s1Val is null && !zeroIfNone) return null;
                Flt64 termVal;
                if (m.Symbol2 is not null)
                {
                    var s2Val = EvaluateSymbolFromResults(m.Symbol2!, results, tokenTable, zeroIfNone);
                    if (s2Val is null && !zeroIfNone) return null;
                    termVal = (s1Val ?? Flt64.Zero) * (s2Val ?? Flt64.Zero);
                }
                else termVal = s1Val ?? Flt64.Zero;
        ...
        public Flt64? Evaluate(IReadOnlyDictionary<ISymbol, Flt64> values, IAbstractTokenTable<Flt64>? tokenTable, IFlt64ValueConverter<Flt64> converter, bool zeroIfNone = false)
        {
            ...
            foreach (var m in _utilsPolynomial.Monomials)
            {
                var s1Val = EvaluateSymbol(m.Symbol1, values, tokenTable, zeroIfNone);
                if (s1Val is null && !zeroIfNone) return null;
                Flt64 termVal;
                if (m.Symbol2 is not null)
                {
                    var s2Val = EvaluateSymbol(m.Symbol2!, values, tokenTable, zeroIfNone);
                    if (s2Val is null && !zeroIfNone) return null;
                    termVal = (s1Val ?? Flt64.Zero) * (s2Val ?? Flt64.Zero);
                }
                else termVal = s1Val ?? Flt64.Zero;
        ...
        public string ToRawString(UInt64 unfold)
        {
            if (unfold != UInt64.Zero)
            {
                var parts = _utilsPolynomial.Monomials
                    ...
                    .Select(m =>
                    {
                        var s1Str = m.Symbol1 switch
                        {
                            IIntermediateSymbol ims => ims.ToRawString(unfold - UInt64.One),
                            ISymbol s => s.Name,
                            _ => "?"
                        };
                        var termStr = m.Symbol2 is not null
                            ? (m.Symbol1 == m.Symbol2 ? $"{s1Str}^2" : $"{s1Str} * {m.Symbol2.Name}")
                            : s1Str;
                        if (m.Coefficient == Flt64.One) return termStr;
                        if (m.Coefficient == -Flt64.One) return $"-{termStr}";
