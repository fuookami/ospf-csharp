...
namespace Fuookami.Ospf.Core.Token
{
    ...
    public sealed class LinearFlattenContext<V> where V : struct, IRing<V>
    {
        ...
        public LinearFlattenData<V>? Get(object cacheKey) => _cache.TryGetValue(cacheKey, out var v) ? v : null;
        ...
        public LinearFlattenData<V>? Remove(object cacheKey) =>
            _cache.Remove(cacheKey, out var v) ? v : null;
    ...
    public sealed class QuadraticFlattenContext<V> where V : struct, IRing<V>
    {
        ...
        public bool Contains(object cacheKey) => _cache.ContainsKey(cacheKey);
        /// <summary>获取缓存值 / Get cached value</summary>
        public QuadraticFlattenData<V>? Get(object cacheKey) => _cache.TryGetValue(cacheKey, out var v) ? v : null;
        /// <summary>放入缓存 / Put into cache</summary>
    ...
    public sealed class ValueCacheContext<V> where V : struct, IRealNumber<V>
    {
        ...
        public V? Value(object cacheKey, IReadOnlyList<V>? solution = null) =>
            _solutionCache.TryGetValue((cacheKey, solution), out var v) ? v : default;
        ...
        public V? Value(object cacheKey, IReadOnlyDictionary<ISymbol, V> fixedValues) =>
            _fixedValueCache.TryGetValue((cacheKey, fixedValues), out var v) ? v : default;
        ...
        public void Remove(object cacheKey)
        {
            ...
            foreach (var key in _solutionCache.Keys)
            {
                if (key.Item1 == cacheKey) solutionKeysToRemove.Add(key);
            }
            foreach (var key in solutionKeysToRemove) _solutionCache.Remove(key);

            var fixedValueKeysToRemove = new List<(object, IReadOnlyDictionary<ISymbol, V>)>();
            foreach (var key in _fixedValueCache.Keys)
            {
                if (key.Item1 == cacheKey) fixedValueKeysToRemove.Add(key);
            }
            foreach (var key in fixedValueKeysToRemove) _fixedValueCache.Remove(key);
        }
    }
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
        ...
        public bool Contains(object cacheKey) => _cache.ContainsKey(cacheKey);
        /// <summary>获取缓存值 / Get cached value</summary>
        public ExpressionRange<V>? Get(object cacheKey) => _cache.TryGetValue(cacheKey, out var v) ? v : null;
        /// <summary>放入缓存 / Put into cache</summary>
