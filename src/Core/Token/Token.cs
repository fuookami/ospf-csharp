...
namespace Fuookami.Ospf.Core.Token
{
    ...
    public sealed class Token<V> where V : struct, IRealNumber<V>
    {
        ...
        internal Flt64? ResultFlt64
        {
            ...
            {
                _result = value;
                foreach (var cb in RefreshCallbacks.Values) cb(value is not null);
            }
        }
        ...
        public Flt64 Random(Random rng)
        {
            ...
            {
                var lower = (long)System.Math.Round(LowerBound!.Value.Unwrap().ToDouble());
                var upper = (long)System.Math.Round(UpperBound!.Value.Unwrap().ToDouble());
            ...
            {
                var lo = LowerBound!.Value.Unwrap().ToDouble();
                var hi = UpperBound!.Value.Unwrap().ToDouble();
