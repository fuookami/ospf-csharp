...
/// <summary>
/// CPLEX 二次求解器（桩实现） / CPLEX quadratic solver (structural stub).
/// 运行时需加载 CPLEX 托管 DLL，否则返回 OREngineEnvironmentLost。
/// Requires CPLEX managed DLL at runtime; returns OREngineEnvironmentLost otherwise.
/// </summary>
public sealed class CplexQuadraticSolver : IQuadraticSolver {
    ...
    public CplexQuadraticSolver(SolverConfig? config = null) {
    ...
    public Task<Result<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>> InvokeAsync(
        ...
        return Task.FromResult<Result<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>>(
            new Failed<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>(
                new Err<ErrorCode>(ErrorCode.OREngineEnvironmentLost, "CPLEX managed DLL not loaded at runtime")));
    }
    ...
    public Task<QuadraticTetradModel> DumpAsync(QuadraticMechanismModel<Flt64> model, CancellationToken cancellationToken = default)
        => throw new System.NotSupportedException("Dump not supported by CplexQuadraticSolver");
