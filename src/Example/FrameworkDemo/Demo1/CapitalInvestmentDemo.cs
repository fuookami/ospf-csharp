...
namespace Fuookami.Ospf.Example.FrameworkDemo.Demo1
{
    ...
    public sealed class CapitalInvestmentDemo
    {
        ...
        {
            // capital >= minCapital
            var capitalConstraint = _capital!.Polynomial.Ge(MinCapital);
            var r1 = _metaModel.AddConstraint(capitalConstraint, group: null, name: "minCapital");
            if (r1.IsFailed) return r1;

            // liability <= maxLiability
            var liabilityConstraint = _liability!.Polynomial.Le(MaxLiability);
            var r2 = _metaModel.AddConstraint(liabilityConstraint, group: null, name: "maxLiability");
            if (r2.IsFailed) return r2;
