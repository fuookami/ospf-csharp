...
namespace Fuookami.Ospf.Utils.Functional
{
        ...
        where C : notnull where E : Error<C>
    {
        ...
        public void ForEach(Action<E> action) { foreach (var e in Errors) action(e); }
    }
    ...
    public static class Results
    {
        ...
        {
            foreach (var block in blocks) { var r = block(); if (r.IsFailed) return r; }
            return OkInstance;
        }
        ...
        {
            foreach (var block in blocks) { var r = await block().ConfigureAwait(false); if (r.IsFailed) return r; }
            return OkInstance;
        }
        ...
        public static async Task<Result<T, ErrorCode, Error<ErrorCode>>> SyncRun<T>(Func<Task<Result<Success, ErrorCode, Error<ErrorCode>>>>[] blocks, Func<Task<Result<T, ErrorCode, Error<ErrorCode>>>> lastBlock)
        {
            foreach (var block in blocks)
            {
                var r = await block().ConfigureAwait(false);
                if (r.IsFailed) { return new Failed<T, ErrorCode, Error<ErrorCode>>(ExtractError(r)); }
            }
    ...
    public static class ResultExtensions
    {
        ...
        public static void IfOk<T, C, E>(this ExResult<T, C, E> r, Action<Ok<T, C, E>> a) where C : notnull where E : Error<C> { if (r is Ok<T, C, E> ok) a(ok); }
        public static void IfFailed<T, C, E>(this ExResult<T, C, E> r, Action<Failed<T, C, E>> a) where C : notnull where E : Error<C> { if (r is Failed<T, C, E> f) a(f); }
        public static void IfFatal<T, C, E>(this ExResult<T, C, E> r, Action<Fatal<T, C, E>> a) where C : notnull where E : Error<C> { if (r is Fatal<T, C, E> fat) a(fat); }
        public static void IfWarned<T, C, E>(this ExResult<T, C, E> r, Action<Warn<T, C, E>> a) where C : notnull where E : Error<C> { if (r is Warn<T, C, E> w) a(w); }
