...
internal static class Common {
    ...
    public static async Task<List<TResult>> ExecuteWithWorkerPool<TSource, TResult>(
        ...
        if (concurrent >= items.Count) {
            // Parallel WhenAll
            IEnumerable<Task<TResult>> tasks = items.Select((item, i) => func(i, item));
    ...
    public static async Task<Result<List<TResult>, ErrorCode, Error<ErrorCode>>> ExecuteTryWithWorkerPool<TSource, TResult>(
        ...
        var items = source.ToList();
        if (items.Count == 0) {
            return Results.Ok(new List<TResult>());
        }

        List<Result<TResult, ErrorCode, Error<ErrorCode>>> results = await source.ExecuteWithWorkerPool(concurrent, func).ConfigureAwait(false);

        var okResults = new List<TResult>();
        foreach (Result<TResult, ErrorCode, Error<ErrorCode>>? r in results) {
            if (r.IsFailed) {
