...
namespace Fuookami.Ospf.Utils.Parallel
{
    ...
    public static class ParallelAssociateExtensions
    {
            ...
            where TKey : notnull
        {
            ...
            var keys = await items.MapParallelly(keySelector, concurrency).ConfigureAwait(false);
        ...
        {
            var items = source.ToList();
            var values = await items.MapParallelly(valueSelector, concurrency).ConfigureAwait(false);
