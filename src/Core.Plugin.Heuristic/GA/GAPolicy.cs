...
public sealed class GAPolicy : HeuristicPolicy, IGAPolicy {
    ...
    public List<Chromosome> Cross(List<Chromosome> parents) {
        ...
        for (int i = 0; i < parents.Count - 1; i += 2) {
            ...
            int point = _random.Next(1, parent1.Solution.Count);
    ...
    public Chromosome Mutate(Chromosome individual, Func<IReadOnlyList<Flt64>, Flt64?> objectiveFunc) {
        ...
        for (int i = 0; i < individual.Solution.Count; i++) {
            if (_random.NextDouble() < _mutationRate) {
                double noise = SampleGaussian() * _mutationStrength;
                Flt64 newValue = individual.Solution[i].Plus(new Flt64(noise));
                double clamped = System.Math.Clamp(newValue.ToDouble(), -100.0, 100.0);
        ...
        Flt64 newObj = objectiveFunc(solution) ?? individual.Objective;
