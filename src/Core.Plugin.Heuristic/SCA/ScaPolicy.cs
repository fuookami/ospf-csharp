...
namespace Fuookami.Ospf.Core.Plugin.Heuristic.Sca
{
        ...
        where V : struct, IRealNumber<V>, INumberField<V>
    {
        ...
        {
            var ratio = MaxIterations > 0 ? (double)iteration.Current / MaxIterations : 0;
        ...
        {
            var dim = solution.Count;
            var newSolution = new List<V>(dim);
            for (int i = 0; i < dim; i++)
            {
                var pos = _fromValue(solution[i]);
                var bestPos = _fromValue(bestSolution[i]);
                var delta = r3[i] * bestPos - pos;
                Flt64 newPos;
