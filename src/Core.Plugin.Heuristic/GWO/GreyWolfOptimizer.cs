...
public sealed class GreyWolfOptimizer : HeuristicAlgorithm<Flt64, Flt64, Flt64> {
    ...
    public override async Task<Result<List<IIndividual<Flt64, Flt64>>, ErrorCode, Error<ErrorCode>>> InvokeAsync(
        ...
        return await Task.Run(() => {
            ...
            IReadOnlyList<IReadOnlyList<Flt64>> initialSolutions = model.InitialSolutions((ulong)PopulationSize);
            ...
            try {
                while (!cancellationToken.IsCancellationRequested) {
                    ...
                    Flt64 a = Policy.A(iteration);
                    Wolf alpha = population[0];
                    Wolf beta = population.Count > 1 ? population[1] : alpha;
                    Wolf delta = population.Count > 2 ? population[2] : beta;

                    var newPopulation = new List<Wolf>(population.Count);
                    foreach (Wolf? wolf in population) {
                        Wolf moved = Policy.Move(iteration, wolf, alpha, beta, delta, a, ObjectiveFunc);
                        Flt64 newObj = ObjectiveFunc(moved.Solution);
                        newPopulation.Add(new Wolf(moved.Solution, newObj, newObj.ToDouble()));
                    }

                    population = newPopulation.OrderBy(x => x, comparer).ToList();
                    Wolf newBest = population[0];
                    ...
                    if (runningCallBack is not null) {
                        Result<Success, ErrorCode, Error<ErrorCode>> cbResult = runningCallBack(iteration, bestWolf, goodWolves);
