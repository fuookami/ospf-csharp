...
namespace Fuookami.Ospf.Core.Plugin.Heuristic.Saa
{
    ...
    public class SaaPolicy : HeuristicPolicy, ISaaPolicy
    {
        ...
        /// <inheritdoc/>
        public Flt64 Temperature(Iteration iteration)
        {
            return new Flt64(_initialTemperature * System.Math.Pow(_temperatureGradient, iteration.Current));
        }

        /// <inheritdoc/>
        public bool Accept(Iteration iteration, Flt64 currentObjective, Flt64 newObjective)
        {
            var delta = (newObjective - currentObjective).Abs();
            var temp = Temperature(iteration);
            if (temp.ToDouble() <= 0) return false;
            var acceptProb = System.Math.Exp(-delta.ToDouble() / temp.ToDouble());
        ...
        {
            return base.Finished(iteration) || Temperature(iteration).ToDouble() <= _finalTemperature;
        }
    }
}
