...
namespace Fuookami.Ospf.Framework.Bpp2d.Domain
{
    ...
    ) where V : struct, IFloatingNumber<V>
    {
        ...
        {
            var zeroX = QuantityArithmetic.ZeroOf(Sheet.Width);
            var zeroY = QuantityArithmetic.ZeroOf(Sheet.Height);
        ...
        {
            var acc = QuantityArithmetic.ZeroOf(SheetArea);
            foreach (var placement in Placements)
            {
        ...
        {
            var used = UsedArea().ConvertTo(SheetArea.Unit);
            if (used.IsFailed)
            {
            ...
            var ratio = used.Value.Value.Div(SheetArea.Value);
