...
public static class MemberPathExtractor {
    ...
    public static PropertyPath Extract<E, T>(Expression<Func<E, T>> lambda) {
        ...
        System.Linq.Expressions.Expression? node = lambda.Body;
...
public static class BooleanExpressionDslExtensions {
    ...
    public static AndExpression And(this BooleanExpression self, BooleanExpression other) {
        ...
        if (self is AndExpression sa) {
            operands.AddRange(sa.Operands);
        }
        else {
            operands.Add(self);
        }

        if (other is AndExpression oa) {
            operands.AddRange(oa.Operands);
        }
        else {
            operands.Add(other);
        }
    ...
    public static OrExpression Or(this BooleanExpression self, BooleanExpression other) {
        var operands = new List<BooleanExpression>();
        if (self is OrExpression so) {
            operands.AddRange(so.Operands);
        }
        else {
            operands.Add(self);
        }

        if (other is OrExpression oo) {
            operands.AddRange(oo.Operands);
        }
        else {
            operands.Add(other);
        }
