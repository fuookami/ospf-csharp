...
public sealed class NumericDispatcher {
    ...
    public INumericOps<T>? OpsFor<T>(Type type) where T : struct
        => _registry.TryGetValue(type, out IBoxedNumericOps? boxed) && boxed is BoxedNumericOps<T> typed ? typed.Inner : null;
    ...
    public object? EvaluateUnary(UnaryOperator operator_, object operand) {
        if (operator_ == UnaryOperator.Positive) {
            return operand;
        }

        if (!_registry.TryGetValue(operand.GetType(), out IBoxedNumericOps? ops)) {
            return null;
        }
    ...
    public object? EvaluateBinary(BinaryOperator operator_, object left, object right) {
        ...
        if (!_registry.TryGetValue(left.GetType(), out IBoxedNumericOps? ops)) {
            return null;
        }
