...
namespace Fuookami.Ospf.Math.Symbol.Expression.Operation
{
    ...
    public static class Normalize
    {
        ...
        public static BooleanExpression Apply(BooleanExpression expr, NormalizeConfig? config = null)
        {
            ...
            var result = expr;
            if (config.Flatten) result = Flatten(result);
            if (config.EliminateDoubleNegation) result = EliminateDoubleNegation(result);
            if (config.ApplyDeMorgan) result = ApplyDeMorgan(result);
            if (config.ConstantFolding) result = ConstantFold(result);
            if (config.Deduplicate) result = Deduplicate(result);
            if (config.SortOperands) result = SortOperands(result);
        ...
        public static BooleanExpression Flatten(BooleanExpression expr)
        {
            if (expr is AndExpression a)
            {
                ...
                {
                    var f = Flatten(op);
                    if (f is AndExpression fa) flattened.AddRange(fa.Operands);
                    else flattened.Add(f);
                }
            ...
            if (expr is OrExpression o)
            {
                ...
                {
                    var f = Flatten(op);
                    if (f is OrExpression fo) flattened.AddRange(fo.Operands);
                    else flattened.Add(f);
                }
        ...
        {
            if (expr is BooleanConstant) return expr;
            ...
            if (expr is AndExpression a)
            {
                ...
                if (filtered.Count == 0) return BooleanConstant.True();
            ...
            if (expr is OrExpression o)
            {
                ...
                if (filtered.Count == 0) return BooleanConstant.False();
                if (filtered.Count == 1) return filtered[0];
            ...
            if (expr is NotExpression n)
            {
                var operand = ConstantFold(n.Operand);
                if (operand is BooleanConstant bc)
                {
                    if (bc.Value is Trivalent.True) return BooleanConstant.False();
                    if (bc.Value is Trivalent.False) return BooleanConstant.True();
        ...
        public static BooleanExpression EliminateDoubleNegation(BooleanExpression expr)
        {
            if (expr is NotExpression n)
            {
                var operand = EliminateDoubleNegation(n.Operand);
        ...
        public static BooleanExpression ApplyDeMorgan(BooleanExpression expr)
        {
            if (expr is NotExpression n)
            {
                var operand = ApplyDeMorgan(n.Operand);
        ...
        {
            if (expr is AndExpression a)
                return new AndExpression(a.Operands.Select(SortOperands).OrderBy(o => StructuralKey(o)).ToList());
            if (expr is OrExpression o)
                return new OrExpression(o.Operands.Select(SortOperands).OrderBy(x => StructuralKey(x)).ToList());
            if (expr is NotExpression n)
        ...
        private static BooleanExpression SimplifySingleOperand(BooleanExpression expr)
        {
            if (expr is AndExpression a)
            {
                ...
                if (operands.Count == 0) return BooleanConstant.True();
            ...
            if (expr is OrExpression o)
            {
                ...
                if (operands.Count == 0) return BooleanConstant.False();
                if (operands.Count == 1) return operands[0];
    ...
    public static class StructuralKeyExtensions
    {
        ...
        private static string StructuralKeyFallback(BooleanExpression expr)
        {
            ...
            if (type.IsGenericType)
            {
                ...
                {
                    var op = type.GetProperty("Operator")!.GetValue(expr)!;
                ...
                {
                    var negated = (bool)type.GetProperty("Negated")!.GetValue(expr)!;
                    ...
                    var cands = candidates.Cast<object>().Select(x => ((ScalarExpression<object?>)x).StructuralKey());
                    return $"In:{negated}:{value.StructuralKey()}:{string.Join(",", cands)}";
                }
                if (genDef == typeof(PatternMatch<>))
                {
                    var mode = type.GetProperty("Mode")!.GetValue(expr)!;
                    var negated = (bool)type.GetProperty("Negated")!.GetValue(expr)!;
                    var value = (ScalarExpression<object?>)type.GetProperty("Value")!.GetValue(expr)!;
