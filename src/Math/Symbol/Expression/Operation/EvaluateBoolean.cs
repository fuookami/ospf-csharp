...
namespace Fuookami.Ospf.Math.Symbol.Expression.Operation
{
    ...
    public sealed class MapEvaluationContext : IEvaluationContext
    {
        ...
        public object? this[PropertyPath path] => _values.TryGetValue(path, out var v) ? v : null;
    ...
    public static class EvaluateBoolean
    {
        ...
        {
            if (expr is BooleanConstant c) return c.Value;
            if (expr is Comparison<object> co) return EvaluateComparison(co, context);
            if (expr is InExpression<object> ie) return EvaluateIn(ie, context);
            if (expr is PatternMatch<object> pm) return EvaluatePatternMatch(pm, context);
            if (expr is NullCheck nc) return EvaluateNullCheck(nc, context);
            if (expr is AndExpression a) return EvaluateAnd(a, context);
            if (expr is OrExpression o) return EvaluateOr(o, context);
            if (expr is NotExpression n) return EvaluateNot(n, context);
        ...
        public static bool? EvaluateOrNull(BooleanExpression expr, IEvaluationContext context)
        {
            ...
            if (result is Trivalent.True) return true;
            if (result is Trivalent.False) return false;
            return null;
        }

        private static Trivalent EvaluateComparison(Comparison<object> expr, IEvaluationContext context)
        {
            var leftValue = EvaluateScalar(expr.Left, context);
            if (leftValue is null) return new Trivalent.Unknown();
            var rightValue = EvaluateScalar(expr.Right, context);
            if (rightValue is null) return new Trivalent.Unknown();

            var result = CompareValues(leftValue, rightValue, expr.Operator);
        ...
        {
            var value = EvaluateScalar(expr.Value, context);
            if (value is null) return new Trivalent.Unknown();

            var isIn = false;
            foreach (var candidateExpr in expr.Candidates)
            {
                var candidate = EvaluateScalar(candidateExpr, context);
        ...
        {
            var value = EvaluateScalar(expr.Value, context)?.ToString();
            if (value is null) return new Trivalent.Unknown();
            var pattern = EvaluateScalar(expr.Pattern, context)?.ToString();
            if (pattern is null) return new Trivalent.Unknown();
        ...
        private static Trivalent EvaluateAnd(AndExpression expr, IEvaluationContext context)
        {
            ...
            foreach (var operand in expr.Operands)
            {
                ...
                if (result is Trivalent.False) return new Trivalent.False();
                if (result is Trivalent.Unknown) hasUnknown = true;
            }
        ...
        private static Trivalent EvaluateOr(OrExpression expr, IEvaluationContext context)
        {
            ...
            {
                var result = Evaluate(operand, context);
                if (result is Trivalent.True) return new Trivalent.True();
                if (result is Trivalent.Unknown) hasUnknown = true;
            }
        ...
        {
            var inner = Evaluate(expr.Operand, context);
            if (inner is Trivalent.True) return new Trivalent.False();
            if (inner is Trivalent.False) return new Trivalent.True();
            return new Trivalent.Unknown();
        }

        private static object? EvaluateScalar(ScalarExpression<object> expr, IEvaluationContext context)
        {
            if (expr is ScalarConstant<object> sc) return sc.Value;
            if (expr is ScalarReference<object> sr) return context[sr.Path];
            if (expr is ScalarSymbolReference<object>) return null;
            if (expr is ScalarUnary<object> su)
            {
                var operand = EvaluateScalar(su.Operand, context);
            ...
            {
                var left = EvaluateScalar(sb.Left, context);
                var right = EvaluateScalar(sb.Right, context);
        ...
        private static bool? CompareValues(object left, object right, ComparisonOperator op)
        {
            ...
            switch (op)
            {
                ...
                case ComparisonOperator.Lt: { var c = CompareOrder(left, right); return c.HasValue ? c.Value < 0 : null; }
                case ComparisonOperator.Le: { var c = CompareOrder(left, right); return c.HasValue ? c.Value <= 0 : null; }
                case ComparisonOperator.Gt: { var c = CompareOrder(left, right); return c.HasValue ? c.Value > 0 : null; }
                case ComparisonOperator.Ge: { var c = CompareOrder(left, right); return c.HasValue ? c.Value >= 0 : null; }
                default: return null;
            }
        }
        ...
        private static bool MatchLike(string value, string pattern)
        {
            ...
            foreach (var c in pattern)
            {
                switch (c)
                {
                    ...
                    default:
                        if (char.IsLetterOrDigit(c)) regexPattern.Append(c);
    ...
    public sealed class DefaultScalarFunctionEvaluator : IScalarFunctionEvaluator
    {
        ...
        private static object? EvaluateAbs(IReadOnlyList<object?> arguments)
        {
            ...
            if (value is int i) return global::System.Math.Abs(i);
            if (value is long l) return global::System.Math.Abs(l);
            if (value is float f) return global::System.Math.Abs(f);
            if (value is double d) return global::System.Math.Abs(d);
            if (value is short s) return (short)global::System.Math.Abs(s);
            if (value is decimal dc) return global::System.Math.Abs(dc);
            return null;
        }
