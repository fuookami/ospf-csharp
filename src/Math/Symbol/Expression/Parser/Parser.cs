...
public sealed class Parser {
    ...
    public RetBool Parse() {
        ...
        RetBool orResult = ParseOrExpression();
        if (orResult.IsFailed) {
            return orResult;
        }

        BooleanExpression expr = orResult.Value;
    ...
    private RetBool ParseOrExpression() {
        RetBool leftResult = ParseAndExpression();
        if (leftResult.IsFailed) {
            return leftResult;
        }

        BooleanExpression left = leftResult.Value;

        while (CurrentToken().Type == TokenType.Or) {
            Advance();
            RetBool rightResult = ParseAndExpression();
    ...
    private RetBool ParseAndExpression() {
        RetBool leftResult = ParseNotExpression();
        if (leftResult.IsFailed) {
            return leftResult;
        }
        ...
        while (CurrentToken().Type == TokenType.And) {
            Advance();
            RetBool rightResult = ParseNotExpression();
            if (rightResult.IsFailed) {
                return rightResult;
            }
    ...
    private RetBool ParseNotExpression() {
        if (CurrentToken().Type == TokenType.Not) {
            Advance();
            RetBool operandResult = ParseNotExpression();
    ...
    private RetBool ParseParenthesized() {
        Advance(); // skip (
        RetBool exprResult = ParseOrExpression();
        if (exprResult.IsFailed) {
            return exprResult;
        }

        RetToken expectResult = Expect(TokenType.RParen, "Expected ')'");
        if (expectResult.IsFailed) {
            return PropagateError<Token, BooleanExpression>(expectResult);
        }
    ...
    private RetBool ParsePathExpression() {
        RetPath pathResult = ParsePath();
        if (pathResult.IsFailed) {
            return PropagateError<PropertyPath, BooleanExpression>(pathResult);
        }

        PropertyPath path = pathResult.Value;
    ...
    private RetBool ParseNullCheck(PropertyPath path) {
        ...
        RetToken expectResult = Expect(TokenType.Null, "Expected 'null' after 'is'");
        if (expectResult.IsFailed) {
            return PropagateError<Token, BooleanExpression>(expectResult);
        }
    ...
    private RetBool ParseInExpression(PropertyPath path, bool negated) {
        RetToken expectLp = Expect(TokenType.LParen, "Expected '(' after 'in'");
        if (expectLp.IsFailed) {
            return PropagateError<Token, BooleanExpression>(expectLp);
        }

        var candidates = new List<ScalarExpression<object?>>();
        while (true) {
            RetScalar candResult = ParseScalarValue();
            if (candResult.IsFailed) {
                return PropagateError<ScalarExpression<object?>, BooleanExpression>(candResult);
            }
        ...
        RetToken expectRp = Expect(TokenType.RParen, "Expected ')' after 'in' list");
        if (expectRp.IsFailed) {
            return PropagateError<Token, BooleanExpression>(expectRp);
        }
    ...
    private RetBool ParsePatternMatch(PropertyPath path, PatternMatchMode mode, bool negated) {
        RetScalar patternResult = ParseScalarValue();
        if (patternResult.IsFailed) {
            return PropagateError<ScalarExpression<object?>, BooleanExpression>(patternResult);
        }
    ...
    private RetBool ParseComparison(PropertyPath leftPath) {
        ...
        RetScalar rightResult = ParseScalarValue();
        if (rightResult.IsFailed) {
            return PropagateError<ScalarExpression<object?>, BooleanExpression>(rightResult);
        }
    ...
    private RetScalar ParseScalarValue() {
        ...
        if (tt == TokenType.Number) {
            string v = CurrentToken().Value;
            Advance();
            bool parsed = double.TryParse(v, NumberStyles.Float, CultureInfo.InvariantCulture, out double d);
        ...
        if (tt == TokenType.Identifier) {
            RetPath pathResult = ParsePath();
            if (pathResult.IsFailed) {
                return PropagateError<PropertyPath, ScalarExpression<object?>>(pathResult);
            }
    ...
    private OrExpression MergeOr(BooleanExpression left, BooleanExpression right) {
        ...
        if (left is OrExpression lo) {
            operands.AddRange(lo.Operands);
        }
        else {
            operands.Add(left);
        }

        if (right is OrExpression ro) {
            operands.AddRange(ro.Operands);
        }
        else {
            operands.Add(right);
        }
    ...
    private AndExpression MergeAnd(BooleanExpression left, BooleanExpression right) {
        var operands = new List<BooleanExpression>();
        if (left is AndExpression la) {
            operands.AddRange(la.Operands);
        }
        else {
            operands.Add(left);
        }

        if (right is AndExpression ra) {
            operands.AddRange(ra.Operands);
        }
        else {
            operands.Add(right);
        }
...
public static class BooleanExpressionParser {
    ...
    public static RetBool Parse(string input) {
        IReadOnlyList<Token> tokens = new Lexer(input).Tokenize();
