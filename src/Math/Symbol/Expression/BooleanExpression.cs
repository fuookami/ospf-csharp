...
public sealed record Comparison<T>(
    ...
    public override void CollectReferencesInto(HashSet<PropertyPath> refs) {
        if (Left is ScalarReference<T> lr) {
            refs.Add(lr.Path);
        }

        if (Right is ScalarReference<T> rr) {
            refs.Add(rr.Path);
        }
...
public sealed record InExpression<T>(
    ...
    public override void CollectReferencesInto(HashSet<PropertyPath> refs) {
        ...
        foreach (ScalarExpression<T> c in Candidates) {
            c.CollectReferencesInto(refs);
        }
    }
...
public sealed record PatternMatch<T>(
    ...
    public override bool IsConstant() => Value.IsConstant();
    /// <inheritdoc/>
    public override void CollectReferencesInto(HashSet<PropertyPath> refs) => Value.CollectReferencesInto(refs);
    /// <inheritdoc/>
    public override string ToString() => Negated
...
public sealed record AndExpression : BooleanExpression {
    ...
    /// <inheritdoc/>
    public override void CollectReferencesInto(HashSet<PropertyPath> refs) {
        foreach (BooleanExpression o in Operands) {
            o.CollectReferencesInto(refs);
        }
    }
    /// <inheritdoc/>
    ...
    public AndExpression(IReadOnlyList<BooleanExpression> operands) {
        if (operands.Count == 0) {
            throw new ArgumentException("And expression requires at least one operand", nameof(operands));
        }
...
public sealed record OrExpression : BooleanExpression {
    ...
    public OrExpression(IReadOnlyList<BooleanExpression> operands) {
        if (operands.Count == 0) {
            throw new ArgumentException("Or expression requires at least one operand", nameof(operands));
        }
