...
namespace Fuookami.Ospf.Math.Symbol.Operation
{
    ...
    public static class FactorizationOps
    {
            ...
            where T : struct, IRing<T>, IField<T>, IFloatingNumber<T>
        {
            ...
            var discriminant = b.Times(b).Minus(four.Times(a).Times(c));
            ...
            if (discriminant.Eq(zero))
            {
                ...
                var root = b.Negate().Div(two.Times(a));
                return new PolynomialRoots<T>(true, new[] { root });
            }

            // Two real roots: (-b +/- sqrt(d)) / (2a)
            var sqrtD = discriminant.Sqrt();
            var twoA = two.Times(a);
            var root1 = b.Negate().Plus(sqrtD).Div(twoA);
            var root2 = b.Negate().Minus(sqrtD).Div(twoA);
        ...
        public static QuadraticPolynomial<T> Expand<T>(this QuadraticFactorization<T> factorization)
            where T : struct, IRing<T>, IRealNumber<T>
        {
            var lc = factorization.LeadingCoefficient;
            var symbol = factorization.Symbol;
            var constants = NumericConstantsRegistry.For<T>();
            var zero = constants.Zero;
            ...
            {
                // a(x - r) = ax - ar
                var root = factorization.Factors[0].Root;
                var linear = QuadraticMonomial<T>.Linear(lc, symbol);
                var constant = lc.Negate().Times(root);
            ...
            if (factorization.Factors.Count == 2)
            {
                ...
                var r1 = factorization.Factors[0].Root;
                var r2 = factorization.Factors[1].Root;

                var quad = QuadraticMonomial<T>.Quadratic(lc, symbol, symbol);
                var linearCoeff = lc.Negate().Times(r1.Plus(r2));
                var linear = QuadraticMonomial<T>.Linear(linearCoeff, symbol);
                var constant = lc.Times(r1).Times(r2);
