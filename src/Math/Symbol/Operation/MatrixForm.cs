...
public static class MatrixFormOps {
    ...
    private static Result<int, ErrorCode, Error<ErrorCode>> RequireSymbolIndex(
        ...
        return indexOfSymbol.TryGetValue(symbol, out int i)
    ...
    public static Result<LinearMatrixForm<T>, ErrorCode, Error<ErrorCode>> ToMatrixForm<T>(
        ...
        T zero)
        where T : struct, IRing<T>, IRealNumber<T> {
        Result<Success, ErrorCode, Error<ErrorCode>> validation = ValidateOrder(order);
        ...
        LinearPolynomial<T> source = p.CombineLinearTerms(zero);
        ...
        foreach (LinearMonomial<T> m in source.Monomials) {
            Result<int, ErrorCode, Error<ErrorCode>> idxResult = RequireSymbolIndex(m.Symbol, indexOfSymbol);
    ...
    public static Result<QuadraticMatrixForm<T>, ErrorCode, Error<ErrorCode>> ToMatrixForm<T>(
        ...
        QuadraticPolynomial<T> source = p.CombineQuadraticTerms(zero);
        int n = order.Count;
        var q = new T[n][];
        for (int i = 0; i < n; i++) {
            q[i] = new T[n];
        ...
        foreach (QuadraticMonomial<T> m in source.Monomials) {
            if (m.IsQuadratic) {
                Result<int, ErrorCode, Error<ErrorCode>> iResult = RequireSymbolIndex(m.Symbol1, indexOfSymbol);
                if (iResult.IsFailed) {
                    return new Failed<QuadraticMatrixForm<T>, ErrorCode, Error<ErrorCode>>(
                        ((Failed<int, ErrorCode, Error<ErrorCode>>)iResult).Error);
                }

                Result<int, ErrorCode, Error<ErrorCode>> jResult = RequireSymbolIndex(m.Symbol2!, indexOfSymbol);
                if (jResult.IsFailed) {
                    return new Failed<QuadraticMatrixForm<T>, ErrorCode, Error<ErrorCode>>(
                ...
                if (i == j) {
                    ...
                    (T left, T right) = splitOffDiagonal(m.Coefficient);
    ...
    public static QuadraticPolynomial<T> QuadraticPolynomialFromMatrixForm<T>(
        ...
        for (int i = 0; i < order.Count; i++) {
            ...
            for (int j = i + 1; j < order.Count; j++) {
                T coefficient = mergeOffDiagonal(q[i][j], q[j][i]);
    ...
    public static Result<QuadraticMatrixForm<T>, ErrorCode, Error<ErrorCode>> ToMatrixForm<T>(
        ...
        System.Func<T, (T Left, T Right)> splitOffDiagonal)
        where T : struct, IRing<T>, IRealNumber<T> {
        QuadraticPolynomial<T>? quadratic = p.ToQuadraticPolynomialOrNull();
