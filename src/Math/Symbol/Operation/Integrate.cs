...
public static class IntegrateOps {
    ...
    public static CanonicalPolynomial<T> Integrate<T>(
        ...
        foreach (QuadraticMonomial<T> m in p.Monomials) {
            if (m.IsQuadratic) {
                bool s1Match = SymbolIdentity.StableId(m.Symbol1) == SymbolIdentity.StableId(variable);
                bool s2Match = SymbolIdentity.StableId(m.Symbol2!) == SymbolIdentity.StableId(variable);
                ...
                if (s1Match && s2Match) {
                    ...
                    ISymbol otherSymbol = s1Match ? m.Symbol2! : m.Symbol1;
                    powers[otherSymbol] = powers.TryGetValue(otherSymbol, out int p1) ? p1 + 1 : 1;
                    powers[variable] = powers.TryGetValue(variable, out int pv) ? pv + 1 : 1;
                    T two = constants.Two;
