...
public static class ParseOps {
    ...
    public static LinearPolynomial<Flt64>? ParseLinearFlt64(
        ...
        List<(Flt64 Coefficient, List<ISymbol> Symbols)>? terms = ParseTerms(text, symbolMap);
        if (terms is null) {
            return null;
        }
        ...
        foreach ((Flt64 coeff, List<ISymbol>? symbols) in terms) {
            if (symbols.Count == 0) {
    ...
    public static LinearInequality<Flt64>? ParseLinearInequalityFlt64(
        string text,
        IReadOnlyDictionary<string, ISymbol> symbolMap) {
        (string Lhs, Comparison? Comp, string Rhs)? split = SplitInequality(text);
        if (split is null) {
            return null;
        }

        (string? lhsText, Comparison? comp, string? rhsText) = split.Value;

        LinearPolynomial<Flt64>? lhs = ParseLinearFlt64(lhsText, symbolMap);
        LinearPolynomial<Flt64>? rhs = ParseLinearFlt64(rhsText, symbolMap);
    ...
    private static List<(Flt64 Coefficient, List<ISymbol> Symbols)>? ParseTerms(
        ...
        foreach (string token in tokens) {
            (Flt64? coeff, List<ISymbol>? symbols) = ParseSingleTerm(token, symbolMap);
            if (coeff is null) {
                return null;
            }
    ...
    private static (Flt64? Coefficient, List<ISymbol> Symbols) ParseSingleTerm(
        ...
        if (remaining.Contains('*')) {
            ...
            foreach (string part in parts) {
                if (symbolMap.TryGetValue(part.Trim(), out ISymbol? sym)) {
                    symbols.Add(sym);
                }
            ...
            foreach ((string? name, ISymbol? sym) in symbolMap) {
