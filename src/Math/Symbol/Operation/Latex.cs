...
namespace Fuookami.Ospf.Math.Symbol.Operation
{
    ...
    public static class LatexOps
    {
            ...
            where T : struct, IRing<T>, IRealNumber<T>
        {
            ...
            var coeff = numberOps.Format(m.Coefficient);
            var sym = m.Symbol.Name;
            ...
            where T : struct, IRing<T>, IRealNumber<T>
        {
            options ??= LatexOptions.Default;
            var coeff = numberOps.Format(m.Coefficient);
            var sym1 = m.Symbol1.Name;

            if (m.IsQuadratic)
            {
                var sym2 = m.Symbol2!.Name;
                var body = sym1 == sym2 ? $"{sym1}^{{2}}" : $"{sym1}{sym2}";
                if (options.UseTimesSymbol)
            ...
            where T : struct, IRing<T>, IRealNumber<T>
        {
            ...
            {
                var term = m.ToLatexString(numberOps, options);
            ...
            // Constant
            var constants = p.Constant.Constants;
            if (!p.Constant.Eq(p.Constant.Minus(p.Constant)))
            {
                var constStr = numberOps.Format(p.Constant);
            ...
            where T : struct, IRing<T>, IRealNumber<T>
        {
            ...
            var zero = p.Constant.Minus(p.Constant);
        ...
        public static string ToLatexString<T>(
            ...
            where T : struct, IRing<T>, IRealNumber<T>
        {
            var lhs = ineq.Lhs.ToLatexString(numberOps, options);
            var rhs = ineq.Rhs.ToLatexString(numberOps, options);
            var op = ineq.Comparison switch
            {
                Comparison.LT => " < ",
                Comparison.LE => " \\leq ",
                Comparison.EQ => " = ",
