...
namespace Fuookami.Ospf.Math.Symbol.Monomial
{
    ...
    public sealed class PowerVectorKey
    {
            ...
            int totalSymbols)
        {
            ...
            var sparsity = (double)size / totalSymbols;
            if (totalSymbols <= 5 || sparsity >= SparsityThreshold)
            {
                var vec = new int[totalSymbols];
                foreach (var (s, p) in powers) vec[symbolIndex[s]] = p;
                return Dense(vec);
            }
            else
            {
                var entries = powers.OrderBy(kv => symbolIndex[kv.Key]).ToList();
                var indices = entries.Select(kv => symbolIndex[kv.Key]).ToArray();
                var powersArr = entries.Select(kv => kv.Value).ToArray();
        ...
        public IReadOnlyDictionary<ISymbol, int> ToPowers(IReadOnlyList<ISymbol> symbolList)
        {
            ...
            if (IsDense)
            {
                for (var i = 0; i < _denseVec!.Length; i++)
                    if (_denseVec[i] != 0) result[symbolList[i]] = _denseVec[i];
            }
            else if (IsSparse)
            {
        ...
        public override bool Equals(object? obj)
        {
            ...
            if (obj is not PowerVectorKey other) return false;
            if (Hash != other.Hash) return false;
            if (IsDense && other.IsDense) return _denseVec!.SequenceEqual(other._denseVec!);
