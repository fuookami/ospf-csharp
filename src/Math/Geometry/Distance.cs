...
public sealed record EuclideanMetric : Distance {
    ...
    public override V Compute<D, V>(Point<D, V> lhs, Point<D, V> rhs) {
        IFloatingNumberConstants<V> c = lhs[0].Constants;
        V sum = lhs.Indices.Aggregate(c.Zero, (acc, i) => acc.Plus(lhs[i].Minus(rhs[i]).Sqr()));
        return sum.Sqrt();
    }
}
...
public sealed record MinkowskiMetric(int P) : Distance {
    ...
    public override V Compute<D, V>(Point<D, V> lhs, Point<D, V> rhs) {
        IFloatingNumberConstants<V> c = lhs[0].Constants;
        V sum = lhs.Indices.Aggregate(c.Zero, (acc, i) => acc.Plus(lhs[i].Minus(rhs[i]).Abs().Pow(P)));
        // sum^(1/P) using System.Math for the root
        Flt64 dSum = sum.ToFlt64();
