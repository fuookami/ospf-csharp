...
public sealed record PlaneFrame3
{
    ...
    {
        if (firstAxis == secondAxis) throw new ArgumentException("firstAxis and secondAxis must be different.");
        FirstAxis = firstAxis;
    ...
    public Result<V, ErrorCode, Error<ErrorCode>> Distance<V>(PlanePoint3<V> point) where V : struct, IFloatingNumber<V>
        => NormalAxis().Map(a => point.Along(a));
    ...
    {
        var zero = GeometryOps.ZeroOf(distance);
