...
public sealed record DelaunayTriangulation2(
    ...
    public IReadOnlyList<Edge<Dim2, Flt64>> Edges {
        get {
            ...
            foreach (Triangle<Dim2, Flt64> t in Triangles) {
                foreach (Edge<Dim2, Flt64> e in t.Edges) {
                    (int i1, int i2) = FindPointIndices(e.From, e.To);
    ...
    private (int, int) FindPointIndices(Point<Dim2, Flt64> p1, Point<Dim2, Flt64> p2) {
        ...
        for (int i = 0; i < Points.Count; i++) {
            if (Points[i].ApproxEq(p1)) {
                i1 = i;
            }

            if (Points[i].ApproxEq(p2)) {
                i2 = i;
            }
        }
        return (i1, i2);
...
public static class Delaunay {
    ...
    public static IReadOnlyList<Triangle<Dim2, Flt64>> Invoke(IReadOnlyList<Point<Dim2, Flt64>> points) {
        if (points.Count < 3) {
            return Array.Empty<Triangle<Dim2, Flt64>>();
        }

        var sortedPoints = points.OrderBy(p => p.X().Value).ThenBy(p => p.Y().Value).ToList();
        Triangle<Dim2, Flt64> superTriangle = GetSuperTriangle(sortedPoints);
        ...
        foreach (Point<Dim2, Flt64>? point in sortedPoints) {
            var edges = new List<Edge<Dim2, Flt64>>();
            var thisTriangles = new List<Triangle<Dim2, Flt64>>();

            foreach (Triangle<Dim2, Flt64> triangle in undeterminedTriangles) {
                var circumcircle = Circle<Dim2, Flt64>.CircumcircleOf(triangle);
                Flt64 dx = circumcircle.X().Minus(point.X());
                Flt64 dy = circumcircle.Y().Minus(point.Y());
                Flt64 dist = (dx.Sqr().Plus(dy.Sqr())).Sqrt();
            ...
            List<Edge<Dim2, Flt64>> uniqueEdges = DeleteDuplicateEdges(edges);
    ...
    private static List<Edge<Dim2, Flt64>> DeleteDuplicateEdges(List<Edge<Dim2, Flt64>> edges) {
        bool[] duplication = new bool[edges.Count];
    ...
    private static void UpdateTriangles(
        ...
        List<Edge<Dim2, Flt64>> edges) => triangles.AddRange(edges.Select(e => new Triangle<Dim2, Flt64>(e.From, e.To, point)));
    ...
    private static Triangle<Dim2, Flt64> GetSuperTriangle(IReadOnlyList<Point<Dim2, Flt64>> points) {
        double minX = points.Min(p => p.X().Value);
        double maxX = points.Max(p => p.X().Value);
        double minY = points.Min(p => p.Y().Value);
        double maxY = points.Max(p => p.Y().Value);

        double dx = maxX - minX;
        double dy = maxY - minY;
        double midX = (maxX + minX) / 2.0;
        double midY = (maxY + minY) / 2.0;
        double dMax = global::System.Math.Max(dx, dy);
...
public static class TriangulationFunctions {
    ...
    public static Result<IReadOnlyList<Triangle<Dim3, Flt64>>, ErrorCode, Error<ErrorCode>> Triangulate3D(
        ...
        IReadOnlyList<Triangle<Dim2, Flt64>> tris = Triangulate2(points.Select(p => Point<Dim2, Flt64>.Point2(p.X(), p.Y())).ToList());
    ...
    public static Result<IReadOnlyList<Triangle<Dim3, Flt64>>, ErrorCode, Error<ErrorCode>> TriangulateIsolines(
        ...
        for (int i = 0; i < isolines.Count - 1; i++) {
            (Flt64 z, IReadOnlyList<Point<Dim2, Flt64>>? pts) = isolines[i];
            (Flt64 nz, IReadOnlyList<Point<Dim2, Flt64>>? npts) = isolines[i + 1];
            var points = pts.Select(p => Point<Dim3, Flt64>.Point3(p.X(), p.Y(), z))
                .Concat(npts.Select(p => Point<Dim3, Flt64>.Point3(p.X(), p.Y(), nz))).ToList();
            Result<IReadOnlyList<Triangle<Dim3, Flt64>>, ErrorCode, Error<ErrorCode>> result = Triangulate3D(points);
