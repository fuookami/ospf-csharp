...
public sealed class Point<D, V>
    ...
    public override bool Equals(object? obj) {
        if (obj is not Point<D, V> other) {
            return false;
        }

        if (_coords.Length != other._coords.Length) {
            return false;
        }

        for (int i = 0; i < _coords.Length; i++) {
            if (!_coords[i].Eq(other._coords[i])) {
                return false;
            }
        }
        return true;
    ...
    public bool ApproxEq(Point<D, V> other) {
        ...
        for (int i = 0; i < _coords.Length; i++) {
            if (!_coords[i].Equiv(other._coords[i])) {
                return false;
            }
        }
        return true;
    ...
    public bool ApproxEq(Point<D, V> other, V epsilon) {
        ...
        for (int i = 0; i < _coords.Length; i++) {
            if (!_coords[i].Minus(other._coords[i]).Abs().Leq(epsilon)) {
                return false;
            }
        }
        return true;
    ...
    public V DistanceTo(Point<D, V> other) {
        IFloatingNumberConstants<V> c = _coords[0].Constants;
        V sum = Dimension.Indices.Aggregate(c.Zero, (acc, i) => acc.Plus(_coords[i].Minus(other._coords[i]).Sqr()));
        return sum.Sqrt();
    }
    ...
    public Point<D, V> Midpoint(Point<D, V> other) {
        V two = ((IRealNumberConstants<V>)_coords[0].Constants).Two;
...
public static class PointFactory {
    ...
    public static Point<Dim2, V> point2<V>()
        where V : struct, IFloatingNumber<V> {
        IFloatingNumberConstants<V> c = default(V).Constants;
