...
namespace Fuookami.Ospf.Math.Algebra.Law
{
    ...
    public sealed class RingLaw<TSelf>
    {
        ...
        public bool CheckAdditiveCommutative()
        {
            foreach (var lhs in _samples)
                foreach (var rhs in _samples)
                    if (!_equal(_add(lhs, rhs), _add(rhs, lhs))) return false;
            return true;
        }
        ...
        public bool CheckMultiplicativeAssociative()
        {
            foreach (var lhs in _samples)
                foreach (var mid in _samples)
                    foreach (var rhs in _samples)
                        if (!_equal(_mul(_mul(lhs, mid), rhs), _mul(lhs, _mul(mid, rhs)))) return false;
            return true;
        }
        ...
        public bool CheckMultiplicativeIdentity()
        {
            ...
            {
                if (!_equal(_mul(v, _one), v)) return false;
                if (!_equal(_mul(_one, v), v)) return false;
            }
            return true;
        }
        ...
        public bool CheckDistributive()
        {
            foreach (var lhs in _samples)
                foreach (var mid in _samples)
                    foreach (var rhs in _samples)
                    {
                        if (!_equal(_mul(lhs, _add(mid, rhs)), _add(_mul(lhs, mid), _mul(lhs, rhs)))) return false;
                        if (!_equal(_mul(_add(lhs, mid), rhs), _add(_mul(lhs, rhs), _mul(mid, rhs)))) return false;
                    }
            return true;
        }
