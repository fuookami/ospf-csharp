...
public sealed class GroupLaw<TSelf> {
        ...
        foreach (TSelf? lhs in _samples) {
            foreach (TSelf? mid in _samples) {
                foreach (TSelf? rhs in _samples) {
                    if (!_equal(_add(_add(lhs, mid), rhs), _add(lhs, _add(mid, rhs)))) {
                        return false;
                    }
                }
            }
    ...
    public bool CheckIdentity() {
        foreach (TSelf? v in _samples) {
            if (!_equal(_add(v, _zero), v)) {
                return false;
            }

            if (!_equal(_add(_zero, v), v)) {
                return false;
            }
        }
        return true;
    ...
    public bool CheckInverse() {
        foreach (TSelf? v in _samples) {
            TSelf? inv = _negate(v);
            if (!_equal(_add(v, inv), _zero)) {
                return false;
            }

            if (!_equal(_add(inv, v), _zero)) {
                return false;
            }
        }
        return true;
