...
namespace Fuookami.Ospf.Math.Algebra.ValueRange
{
        ...
        where T : struct, IRealNumber<T>, INumberField<T>
    {
        ...
        {
            var c = Constants;
            if (rhs.Equals(c.PositiveInfinity)) return this is Infinity;
            if (rhs.Equals(c.NegativeInfinity)) return this is NegativeInfinity;
            if (c.IsNaN(rhs)) return null;
        ...
        {
            var c = NumericConstantsRegistry.For<T>();
            if (value.Equals(c.PositiveInfinity)) return new Ok<ValueWrapper<T>, ErrorCode, Error<ErrorCode>>(new Infinity());
            if (value.Equals(c.NegativeInfinity)) return new Ok<ValueWrapper<T>, ErrorCode, Error<ErrorCode>>(new NegativeInfinity());
            if (c.IsNaN(value)) return new Failed<ValueWrapper<T>, ErrorCode, Error<ErrorCode>>(ErrorCode.IllegalArgument, "Illegal argument NaN for value range!!!");
        ...
        public sealed record Infinity : ValueWrapper<T>
        {
            ...
            {
                if (Constants.IsNaN(rhs)) return null;
                if (rhs.Equals(Constants.NegativeInfinity)) return new NegativeInfinity();
                if (rhs.Equals(Constants.PositiveInfinity)) return new Infinity();
                if (rhs.Equals(Constants.Zero)) return new Value(Constants.Zero);
            ...
            public override ValueWrapper<T>? Div(T rhs)
            {
                if (Constants.IsNaN(rhs) || rhs.Equals(Constants.PositiveInfinity)
                    || rhs.Equals(Constants.NegativeInfinity) || rhs.Equals(Constants.Zero)) return null;
                return rhs.Ls(Constants.Zero) ? new NegativeInfinity() : new Infinity();
            }
            public override ValueWrapper<T>? Div(ValueWrapper<T> rhs) => rhs switch
            {
        ...
        public sealed record NegativeInfinity : ValueWrapper<T>
        {
            ...
            {
                if (Constants.IsNaN(rhs)) return null;
                if (rhs.Equals(Constants.NegativeInfinity)) return new Infinity();
                if (rhs.Equals(Constants.PositiveInfinity)) return new NegativeInfinity();
                if (rhs.Equals(Constants.Zero)) return new Value(Constants.Zero);
            ...
            public override ValueWrapper<T>? Div(T rhs)
            {
                if (Constants.IsNaN(rhs) || rhs.Equals(Constants.NegativeInfinity)
                    || rhs.Equals(Constants.PositiveInfinity) || rhs.Equals(Constants.Zero)) return null;
                return rhs.Ls(Constants.Zero) ? new Infinity() : new NegativeInfinity();
            }
            public override ValueWrapper<T>? Div(ValueWrapper<T> rhs) => rhs switch
            {
    ...
    internal sealed class ValueWrapperJsonConverterFactory : JsonConverterFactory
    {
        ...
        public override JsonConverter? CreateConverter(Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            var converterType = typeof(ValueWrapperJsonConverter<>).MakeGenericType(t);
        ...
        where T : struct, IRealNumber<T>, INumberField<T>
    {
        public override ValueWrapper<T>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (reader.TokenType == JsonTokenType.String)
            {
                ...
                if (s == "inf") return new ValueWrapper<T>.Infinity();
                if (s == "-inf") return new ValueWrapper<T>.NegativeInfinity();
                throw new JsonException($"Unknown ValueWrapper string: {s}");
            }
            var d = reader.GetDouble();
            if (double.IsPositiveInfinity(d)) return new ValueWrapper<T>.Infinity();
            if (double.IsNegativeInfinity(d)) return new ValueWrapper<T>.NegativeInfinity();
            var c = NumericConstantsRegistry.For<T>();
            if (c is IFlt64ValueConverter<T> converter)
            {
                var v = converter.IntoValue(new Flt64(d));
