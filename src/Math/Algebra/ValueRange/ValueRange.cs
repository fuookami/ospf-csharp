...
namespace Fuookami.Ospf.Math.Algebra.ValueRange
{
        ...
        where T : struct, IRealNumber<T>, INumberField<T>
    {
        ...
        {
            if (lb.IsNegativeInfinity) return false;
            if (ub.IsInfinity) return false;
        ...
        {
            var lbi = lbInterval ?? new Interval.Closed();
            var ubi = ubInterval ?? new Interval.Closed();
            var lower = ValueWrapper<T>.Of(lb);
            if (lower is Failed<ValueWrapper<T>, ErrorCode, Error<ErrorCode>> f1) return new Failed<ValueRange<T>, ErrorCode, Error<ErrorCode>>(f1.Error);
            var upper = ValueWrapper<T>.Of(ub);
            if (upper is Failed<ValueWrapper<T>, ErrorCode, Error<ErrorCode>> f2) return new Failed<ValueRange<T>, ErrorCode, Error<ErrorCode>>(f2.Error);
        ...
        public static Result<ValueRange<T>, ErrorCode, Error<ErrorCode>> OfToInfinity(T lb, Interval? lbInterval = null)
        {
            var lbi = lbInterval ?? new Interval.Closed();
            var lower = ValueWrapper<T>.Of(lb);
            if (lower is Failed<ValueWrapper<T>, ErrorCode, Error<ErrorCode>> f) return new Failed<ValueRange<T>, ErrorCode, Error<ErrorCode>>(f.Error);
        ...
        public static Result<ValueRange<T>, ErrorCode, Error<ErrorCode>> OfFromNegativeInfinity(T ub, Interval? ubInterval = null)
        {
            var ubi = ubInterval ?? new Interval.Closed();
            var upper = ValueWrapper<T>.Of(ub);
            if (upper is Failed<ValueWrapper<T>, ErrorCode, Error<ErrorCode>> f) return new Failed<ValueRange<T>, ErrorCode, Error<ErrorCode>>(f.Error);
        ...
        public ValueWrapper<T>? MeanOrNull
        {
            get
            {
                var sum = LowerBound.Value.Plus(UpperBound.Value);
        ...
        public ValueWrapper<T>? GapOrNull
        {
            get
            {
                ...
                var mean = MeanOrNull?.UnwrapOrNull();
                if (mean is null) return null;
                var c = Constants;
                var dp = c.DecimalPrecision ?? c.One;
                var negMean = mean.Value.Negate();
                var absMean = mean.Value.Geq(negMean) ? mean.Value : negMean;
        ...
        public bool Fixed
        {
            get
            {
                if (LowerBound.Interval is not Interval.Closed || UpperBound.Interval is not Interval.Closed) return false;
                if (LowerBound.Value.IsInfinityOrNegativeInfinity || UpperBound.Value.IsInfinityOrNegativeInfinity) return false;
        ...
        {
            if (UpperBound.Value.Ls(rhs.LowerBound.Value) || rhs.UpperBound.Value.Ls(LowerBound.Value)) return null;
            var (newLb, newLbi) = PickLower(rhs);
            var (newUb, newUbi) = PickUpper(rhs);
        ...
        {
            var (newLb, newLbi) = PickMaxLower(rhs);
            var (newUb, newUbi) = PickMinUpper(rhs);
            return Of(newLb, newUb, newLbi, newUbi).Value;
        }
        ...
        {
            if (LowerBound.Value.IsNegativeInfinity && UpperBound.Value.IsInfinity) return true;
        ...
        {
            var lowerCmp = LowerBound.Value.PartialOrd(rhs.LowerBound.Value);
            var lowerContains = lowerCmp switch
            {
                Order.Less => true,
                Order.Greater => false,
                _ => rhs.LowerBound.Interval is Interval.Open || LowerBound.Interval is Interval.Closed,
            };
            if (!lowerContains) return false;
        ...
        public static ValueRange<T>? operator *(ValueRange<T> lhs, ValueRange<T> rhs)
        {
            ...
            if (nonNull.Count != 4) return null;
        ...
        {
            var cmp = LowerBound.Value.PartialOrd(rhs.LowerBound.Value);
            return cmp switch
            {
        ...
        {
            var cmp = UpperBound.Value.PartialOrd(rhs.UpperBound.Value);
            return cmp switch
            {
        ...
        {
            if (LowerBound.Value.IsInfinityOrNegativeInfinity) return (rhs.LowerBound.Value, rhs.LowerBound.Interval);
            if (rhs.LowerBound.Value.IsInfinityOrNegativeInfinity) return (LowerBound.Value, LowerBound.Interval);
            var cmp = LowerBound.Value.PartialOrd(rhs.LowerBound.Value);
            return cmp switch
            {
        ...
        {
            if (UpperBound.Value.IsInfinityOrNegativeInfinity) return (rhs.UpperBound.Value, rhs.UpperBound.Interval);
            if (rhs.UpperBound.Value.IsInfinityOrNegativeInfinity) return (UpperBound.Value, UpperBound.Interval);
            var cmp = UpperBound.Value.PartialOrd(rhs.UpperBound.Value);
            return cmp switch
            {
        ...
        {
            var lb = r.LowerBound.Value.UnwrapOrNull();
            var ub = r.UpperBound.Value.UnwrapOrNull();
            if (lb is not null && value.Ord(lb.Value) is Order.Less) return lb.Value;
            if (ub is not null && value.Ord(ub.Value) is Order.Greater) return ub.Value;
            return value;
        }
    }
    ...
    internal sealed class ValueRangeJsonConverterFactory : JsonConverterFactory
    {
        ...
        public override JsonConverter? CreateConverter(Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            var converterType = typeof(ValueRangeJsonConverter<>).MakeGenericType(t);
        ...
        where T : struct, IRealNumber<T>, INumberField<T>
    {
        public override ValueRange<T>? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            var lower = root.GetProperty("lowerBound").Deserialize<ValueWrapper<T>>(options)!;
            var upper = root.GetProperty("upperBound").Deserialize<ValueWrapper<T>>(options)!;
            var lbi = root.GetProperty("lowerInterval").GetString() == "open"
                ? new Interval.Open() : (Interval)new Interval.Closed();
            var ubi = root.GetProperty("upperInterval").GetString() == "open"
                ? new Interval.Open() : (Interval)new Interval.Closed();
