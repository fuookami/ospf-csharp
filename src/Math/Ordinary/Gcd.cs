...
namespace Fuookami.Ospf.Math.Ordinary
{
    ...
    public static class Gcd
    {
            ...
            where T : struct, IInteger<T>, IMinus<T, T>
        {
            ...
            if (x.Eq(zero)) return y;
            if (y.Eq(zero)) return x;
            var a = x; var b = y;
            while (!b.Eq(zero))
            {
        ...
        {
            var zero = x.Constants.Zero;
            var a = x.Abs(); var b = y.Abs();
            while (!b.Eq(zero)) { var r = a.Rem(b); a = b; b = r; }
            return a;
        }
            ...
            where T : struct, IInteger<T>, IRem<T, T>
        {
            ...
            if (!e.MoveNext()) return constants.One;
            var acc = e.Current;
            while (e.MoveNext()) acc = GcdModImpl(acc, e.Current);
            return acc;
        }
        ...
        {
            var zero = a.Constants.Zero;
            var one = a.Constants.One;
            var oldR = a; var r = b;
            var oldS = one; var s = zero;
            var oldT = zero; var t = one;
            while (!r.Eq(zero))
            {
                var q = oldR.Div(r);
                var nextR = oldR.Minus(q.Times(r)); oldR = r; r = nextR;
                var nextS = oldS.Minus(q.Times(s)); oldS = s; s = nextS;
                var nextT = oldT.Minus(q.Times(t)); oldT = t; t = nextT;
            }
