...
namespace Fuookami.Ospf.Math.Ordinary
{
    ...
    public sealed class PrimeCache
    {
        ...
        private void ExtendSieve(UInt64 @new)
        {
            lock (_lock)
            {
                if (@new.PartialOrd(_current) is not Order.Greater) return;
                var old = _current;
                var oldSize = _current.Eq(UInt64Constants.Instance.Zero)
                    ? UInt64Constants.Instance.Zero
                    : _current.Increment();
                var newSize = (int)@new.Value + 1;
                var newIsPrime = Enumerable.Repeat(true, newSize).ToArray();
                ...
                {
                    if (@new.PartialOrd(UInt64Constants.Instance.Zero) is Order.Greater or Order.Equal) newIsPrime[0] = false;
                    if (@new.PartialOrd(UInt64Constants.Instance.One) is Order.Greater or Order.Equal) newIsPrime[1] = false;
                }
                _isPrime = newIsPrime;
                _current = @new;
                ...
                {
                    if (p.PartialOrd(sqrtLimit) is Order.Greater) break;
                    var start = p.Times(p).PartialOrd(old.Increment()) is Order.Greater
                        ? p.Times(p)
                        : old.Increment().Plus(p).Minus(UInt64Constants.Instance.One).Div(p).Times(p);
                    for (var j = start; j.PartialOrd(@new) is Order.Less or Order.Equal; j = j.Plus(p))
                        newIsPrime[(int)j.Value] = false;
                }
                var begin = old.PartialOrd(UInt64Constants.Instance.Two) is Order.Less
                    ? UInt64Constants.Instance.Two
                    : old.Increment();
                for (var i = begin; i.PartialOrd(@new) is Order.Less or Order.Equal; i = i.Increment())
                    if (newIsPrime[(int)i.Value])
                    {
                        _primes.Add(i);
                        if (i.PartialOrd(sqrtLimit) is Order.Less or Order.Equal)
                        {
                            var startMultiple = i.Times(i).PartialOrd(begin) is Order.Greater
                                ? i.Times(i) : begin;
                            for (var j = startMultiple; j.PartialOrd(@new) is Order.Less or Order.Equal; j = j.Plus(i))
                                newIsPrime[(int)j.Value] = false;
                        }
                    }
            }
        ...
        public IReadOnlyList<UInt64> GetPrimes(UInt64 limit)
        {
            lock (_lock)
            {
                if (limit.PartialOrd(_current) is Order.Greater) ExtendSieve(limit);
        ...
        {
            if (num.PartialOrd(UInt64Constants.Instance.One) is Order.Less or Order.Equal) return false;
            lock (_lock)
            {
                if (num.PartialOrd(_current) is Order.Greater)
                {
                    if (num.PartialOrd(new UInt64(1000000UL)) is Order.Less or Order.Equal)
                    { ExtendSieve(num); return _isPrime[(int)num.Value]; }
        ...
        {
            if (n.PartialOrd(UInt64Constants.Instance.One) is Order.Less or Order.Equal) return false;
            if (n.PartialOrd(UInt64Constants.Instance.Three) is Order.Less or Order.Equal) return true;
            if (n.Rem(UInt64Constants.Instance.Two).Eq(UInt64Constants.Instance.Zero)
                || n.Rem(UInt64Constants.Instance.Three).Eq(UInt64Constants.Instance.Zero)) return false;
            lock (_lock)
                foreach (var p in _primes)
                {
                    if (p.Times(p).PartialOrd(n) is Order.Greater) break;
                    if (n.Rem(p).Eq(UInt64Constants.Instance.Zero)) return false;
                }
            for (var i = UInt64Constants.Instance.Five; i.Times(i).PartialOrd(n) is Order.Less or Order.Equal; i = i.Plus(new UInt64(6UL)))
        ...
        {
            if (limit.PartialOrd(_current) is not Order.Greater) return;
            ...
            for (var i = UInt64Constants.Instance.Two; i.PartialOrd(sqrtLimit) is Order.Less or Order.Equal; i = i.Increment())
                if (_isPrime[(int)i.Value])
                    for (var j = i.Times(i); j.PartialOrd(limit) is Order.Less or Order.Equal; j = j.Plus(i))
                        _isPrime[(int)j.Value] = false;
            _primes.Clear();
            for (var i = UInt64Constants.Instance.Two; i.PartialOrd(limit) is Order.Less or Order.Equal; i = i.Increment())
                if (_isPrime[(int)i.Value]) _primes.Add(i);
            _current = limit;
        }
    }
