...
public static class MinMax {
    ...
    public static T Min<T>(T lhs, params T[] rest) where T : IOrd<T> {
        T min = lhs;
        foreach (T e in rest) {
            if (e.PartialOrd(min) is Order.Less or Order.Equal) {
                min = e;
            }
        }
    ...
    public static T Max<T>(T lhs, params T[] rest) where T : IOrd<T> {
        T max = lhs;
        foreach (T e in rest) {
            if (e.PartialOrd(max) is Order.Greater or Order.Equal) {
                max = e;
            }
        }
    ...
    public static (T Min, T Max) MinMaxValue<T>(T lhs, params T[] rest) where T : IOrd<T> {
        T min = lhs; T max = lhs;
        foreach (T e in rest) {
            if (e.PartialOrd(min) is Order.Less) {
                min = e;
            }

            if (e.PartialOrd(max) is Order.Greater) {
                max = e;
            }
        }
        return (min, max);
    ...
    public static Result<(T Min, T Max), ErrorCode, Error<ErrorCode>> MinMaxSafe<T>(this IEnumerable<T> source) where T : IOrd<T> {
        ...
        T min = e.Current; T max = min;
        while (e.MoveNext()) {
            T v = e.Current; if (v.PartialOrd(min) is Order.Less) {
                min = v;
            }

            if (v.PartialOrd(max) is Order.Greater) {
                max = v;
            }
        }
    ...
    public static (U MinE, U MaxE) MinMaxBy<T, U>(this IEnumerable<U> source, Func<U, T> extractor) where T : IOrd<T> {
        using IEnumerator<U> iter = source.GetEnumerator();
        if (!iter.MoveNext()) {
            throw new InvalidOperationException("Cannot compute minMaxBy of an empty collection.");
        }

        U? minE = iter.Current; U? maxE = minE;
        T min = extractor(minE); T max = min;
        while (iter.MoveNext()) {
            U? e = iter.Current; T v = extractor(e);
    ...
    public static (T Min, T Max) MinMaxExtracted<T, U>(this IEnumerable<U> source, Func<U, T> extractor) where T : IOrd<T> {
        using IEnumerator<U> iter = source.GetEnumerator();
        if (!iter.MoveNext()) {
            throw new InvalidOperationException("Cannot compute minMaxOf of an empty collection.");
        }

        T min = extractor(iter.Current); T max = min;
        while (iter.MoveNext()) {
            T v = extractor(iter.Current);
            if (v.PartialOrd(min) is Order.Less) {
                min = v;
            }
    ...
    public static (T Min, T Max) MinMaxWith<T>(this IEnumerable<T> source, IComparer<T> comparer) {
        ...
        if (!iter.MoveNext()) {
            throw new InvalidOperationException("Cannot compute minMaxWith of an empty collection.");
        }

        T? min = iter.Current; T? max = min;
        while (iter.MoveNext()) {
            T? v = iter.Current;
