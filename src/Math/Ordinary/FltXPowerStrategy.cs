...
namespace Fuookami.Ospf.Math.Ordinary
{
    ...
    public static class FltXPowerStrategy
    {
        ...
        {
            var normalizedDigits = digits <= 0 ? 1 : digits;
            var threshold = Pow.Of(FltXConstants.Instance.Ten, -normalizedDigits, FltXConstants.Instance, normalizedDigits + 1);
        ...
        {
            if (x.PartialOrd(FltXConstants.Instance.Zero) is Order.Less or Order.Equal) return null;
            var normalizedDigits = digits <= 0 ? 1 : digits;
            var scale = normalizedDigits + 1;
            var m = x.WithScale(scale, MidpointRounding.AwayFromZero);
            var k = FltXConstants.Instance.Zero;
            while (m.PartialOrd(FltXConstants.Instance.Two) is Order.Greater or Order.Equal)
            { m = m.Div(FltXConstants.Instance.Two).WithScale(scale, MidpointRounding.AwayFromZero); k = k.Increment(); }
            while (m.PartialOrd(FltXConstants.Instance.One) is Order.Less)
            { m = m.Times(FltXConstants.Instance.Two).WithScale(scale, MidpointRounding.AwayFromZero); k = k.Minus(FltXConstants.Instance.One); }
            var core = LnUnitInterval(m, scale, precision ?? DefaultPrecision(digits), maxIterations);
            var value = core.Value.Plus(k.Times(FltXConstants.Instance.Lg2)).WithScale(scale, MidpointRounding.AwayFromZero);
        ...
        {
            var two = FltXConstants.Instance.Two; var one = FltXConstants.Instance.One;
            var y = x.Minus(one).Div(x.Plus(one)).WithScale(scale, MidpointRounding.AwayFromZero);
            var yPow = y; var value = y; var i = one; var iterations = 0;
            while (iterations < maxIterations)
            {
                yPow = yPow.Times(y).Times(y).WithScale(scale, MidpointRounding.AwayFromZero);
                var denominator = two.Times(i).Plus(one);
                var term = yPow.Div(denominator).WithScale(scale, MidpointRounding.AwayFromZero);
        ...
        public static FltXSeriesResult ExpWithStats(FltX index, int digits, FltX? precision = default, int maxIterations = 8192)
        {
            var normalizedDigits = digits <= 0 ? 1 : digits;
            var scale = normalizedDigits + 1;
            var one = FltXConstants.Instance.One;
            var value = one.WithScale(scale, MidpointRounding.AwayFromZero);
            var term = one.WithScale(scale, MidpointRounding.AwayFromZero);
            var i = one; var iterations = 0;
            while (iterations < maxIterations)
            {
                var next = term.Times(index.WithScale(scale, MidpointRounding.AwayFromZero)).Div(i).WithScale(scale, MidpointRounding.AwayFromZero);
        ...
        public static FltX? PowOrNull(FltX @base, FltX index, int digits, FltX? precision = default, int maxIterations = 8192)
        {
            ...
            var lnBase = Ln(@base, digits, precision, maxIterations);
            if (lnBase is null) return null;
