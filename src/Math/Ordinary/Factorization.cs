...
public static class Factorization {
    ...
    private static IReadOnlyList<(T Prime, int Exponent)> FactorizeWithPrimes<T>(
        ...
        if (num.PartialOrd(constants.One) is Order.Less or Order.Equal) {
            return Array.Empty<(T, int)>();
        }

        T n = num;
        var factors = new List<(T Prime, int Exponent)>();
        foreach (T prime in primes) {
            if (prime.Times(prime).PartialOrd(num) is Order.Greater) {
                break;
            }
            ...
            if (index != 0) {
                factors.Add((prime, index));
            }
        }
        if (n.PartialOrd(constants.One) is Order.Greater) {
            factors.Add((n, 1));
        }
    ...
    internal static IReadOnlyList<(T Prime, int Exponent)> FactorizeImpl<T>(T num, INumericConstants<T> constants)
        where T : struct, IInteger<T>, IDiv<T, T>, IRem<T, T> {
        ulong sqrtULong = num.ToFlt64().Sqrt().Floor().ToUInt64().Value + 1UL;
        T sqrtLimit = constants.One;
        for (ulong count = sqrtULong - 1UL; count > 0UL; count--) {
            sqrtLimit = sqrtLimit.Increment();
        }

        IReadOnlyList<T> primes = Prime.GetPrimesImpl(sqrtLimit, constants);
    ...
    internal static IReadOnlyList<(UInt64 Prime, int Exponent)> FactorizeImpl(UInt64 num, INumericConstants<UInt64> constants) {
        if (num.PartialOrd(UInt64Constants.Instance.One) is Order.Less or Order.Equal) {
            return Array.Empty<(UInt64, int)>();
        }

        UInt64 sqrtLimit = num.ToFlt64().Sqrt().Floor().ToUInt64().Increment();
        IReadOnlyList<UInt64> primes = Prime.GetPrimesUpTo(sqrtLimit);
        return FactorizeWithPrimes(num, primes, constants);
    }
    ...
    public static Result<T, ErrorCode, Error<ErrorCode>> Defactorize<T>(IEnumerable<(T Prime, int Exponent)> factors, INumericConstants<T> constants)
        ...
        T value = constants.One;
        foreach ((T factor, int index) in factors) {
            if (index < 0) {
    ...
    internal static IReadOnlyList<T> DivisorsImpl<T>(IReadOnlyList<(T Prime, int Exponent)> factors, INumericConstants<T> constants)
        ...
        foreach ((T factor, int index) in factors) {
            ...
            T factorPower = constants.One;
            for (int k = 0; k <= index; k++) {
                foreach (T v in values) {
                    next.Add(v.Times(factorPower));
                }
    ...
    public static int DivisorCount<T>(IEnumerable<(T Prime, int Exponent)> factors)
        ...
        int count = 1;
        foreach ((T _, int index) in factors) {
            if (index > 0) {
                count *= (index + 1);
            }
        }
    ...
    internal static T EulerTotientImpl<T>(T num, INumericConstants<T> constants)
        ...
        if (num.Eq(constants.Zero) || num.Eq(constants.One)) {
            return num;
        }

        T value = num;
        T one = constants.One;
        foreach ((T prime, int _) in FactorizeImpl(num, constants)) {
