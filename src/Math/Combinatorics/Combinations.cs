    ...
    public static class Combinations
    {
            ...
            Func<List<T>, bool>? stopped = null)
        {
            ...
            for (var i = 1; i < total; i++)
            {
                ...
                if (stopped?.Invoke(combination) == true) break;
            }
            return result;
        }
        ...
        {
            var result = new List<List<T>>();
            foreach (var combination in CombineSequence(input, choose))
            {
                result.Add(combination);
                callBack?.Invoke(combination);
                if (stopped?.Invoke(combination) == true) break;
        ...
        public static long CombineCount(int n, int choose)
        {
            ...
            if (choose == 0 || choose == n) return 1L;
            var k = global::System.Math.Min(choose, n - choose);
            long value = 1L;
            for (var i = 1; i <= k; i++)
                value = value * (n - k + i) / i;
            return value;
        ...
        public static IEnumerable<List<T>> CombineSequence<T>(IReadOnlyList<T> input)
        {
            for (var k = 1; k <= input.Count; k++)
                foreach (var c in CombineSequence(input, k))
                    yield return c;
        }
