...
namespace Fuookami.Ospf.Math.Combinatorics
{
    ...
    public static class Permutations
    {
            ...
            Func<List<T>, bool>? stopped = null)
        {
            ...
            var p = new int[input.Count];
            var perms = new List<List<T>> { new List<T>(a) };
            callBack?.Invoke(new List<T>(a));
            if (stopped?.Invoke(new List<T>(a)) == true) return perms;

            for (var i = 1; i < input.Count;)
            {
                if (p[i] < i)
                {
                    ...
                    if (stopped?.Invoke(copy) == true) return perms;
                    p[i]++;
                    i = 1;
                }
                else
            ...
            Func<List<T>, bool>? stopped = null)
        {
            ...
            foreach (var permutation in PermuteSequence(input, choose))
            {
                result.Add(permutation);
                callBack?.Invoke(permutation);
                if (stopped?.Invoke(permutation) == true) break;
            }
            return result;
        }
        ...
        {
            if (choose < 0 || choose > n) return 0L;
            long value = 1L;
        ...
        {
            if (choose < 0 || choose > input.Count) yield break;
            if (choose == 0) { yield return new List<T>(); yield break; }

            var used = new bool[input.Count];
            var path = new List<T>(choose);

            foreach (var perm in DfsPermute(input, choose, used, path))
                yield return perm;
        }
