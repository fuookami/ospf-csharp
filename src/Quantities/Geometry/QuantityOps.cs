...
public static class QuantityOps
{
        ...
        where V : struct, IFloatingNumber<V>
    {
        ...
        {
            var ord = lhs.Value.PartialOrd(rhs.Value);
        ...
        var ordConv = converted.Value.Value.PartialOrd(rhs.Value);
    ...
    public static Result<Quantity<V>, ErrorCode, Error<ErrorCode>> ClampSafe<V>(
        ...
        where V : struct, IFloatingNumber<V>
    {
        var lowerOrd = OrdSafe(value, lb, $"{axis}-lb");
        if (lowerOrd is Failed<Order, ErrorCode, Error<ErrorCode>> fl) return Results.Failed<Quantity<V>>(fl.Error);
        if (lowerOrd.Value is Order.Less) return Results.Ok(lb);
    ...
    public static Result<bool, ErrorCode, Error<ErrorCode>> ContainsInRangeSafe<V>(
        ...
        where V : struct, IFloatingNumber<V>
    {
        var lowerOrd = OrdSafe(value, lb, axis);
        if (lowerOrd is Failed<Order, ErrorCode, Error<ErrorCode>> fl) return Results.Failed<bool>(fl.Error);

        var upperOrd = OrdSafe(value, ub, axis);
        if (upperOrd is Failed<Order, ErrorCode, Error<ErrorCode>> fu) return Results.Failed<bool>(fu.Error);

        var lowerOk = withLowerBound
            ? lowerOrd.Value is Order.Equal or Order.Greater
            : lowerOrd.Value is Order.Greater;
        var upperOk = withUpperBound
