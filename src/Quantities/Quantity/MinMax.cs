...
namespace Fuookami.Ospf.Quantities.Quantity
{
    ...
    public static class QuantityMinMax
    {
        ...
        {
            if (lhs is Flt64 f64l && rhs is Flt64 f64r) return f64l.CompareTo(f64r);
            if (lhs is FltX fxl && rhs is FltX fxr) return fxl.CompareTo(fxr);
            if (lhs is Int64 i64l && rhs is Int64 i64r) return i64l.CompareTo(i64r);
            if (lhs is IntX ixl && rhs is IntX ixr) return ixl.CompareTo(ixr);
            if (lhs is UInt64 u64l && rhs is UInt64 u64r) return u64l.CompareTo(u64r);
