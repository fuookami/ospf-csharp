...
namespace Fuookami.Ospf.Quantities.Unit
{
        ...
        IDictionary<DerivedQuantity, PhysicalUnit> DerivedCache) : IUnitSystem
    {
        ...
        {
            if (StandardUnits.TryGetValue(quantity, out var std))
                return std;
        ...
        private PhysicalUnit? UnitForDimension(DerivedQuantity quantity)
        {
            ...
            {
                var fq = quantity.Quantities[0];
                if (fq.Index == 1 && BaseUnits.TryGetValue(fq.Dimension, out var baseUnit))
                    return baseUnit;
            }

            if (DerivedCache.TryGetValue(quantity, out var cached))
                return cached;

            var unit = DeriveUnit(quantity);
            if (unit != null)
        ...
        private PhysicalUnit? DeriveUnit(DerivedQuantity quantity)
        {
            ...
            foreach (var fq in quantity.Quantities)
            {
                if (!BaseUnits.TryGetValue(fq.Dimension, out var baseUnit))
                    return null;
                var power = fq.Index;
                if (power == 0) continue;

                var unitScale = baseUnit.Scale;
                if (power == 1)
                {
                ...
                else
                {
                    ...
                    var absPower = System.Math.Abs(power);
                    ...
                    {
                        var div = resultScale / powScale;
                        if (div == null) return null;
                        resultScale = div;
                    }
                }

                var unitSym = baseUnit.Symbol ?? "";
                if (power == 1)
            ...
            var symbol = string.Join("·", symbolParts);
            var name = $"derived_{quantity.DimensionSymbol()}";
