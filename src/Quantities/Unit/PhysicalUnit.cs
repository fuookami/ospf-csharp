...
namespace Fuookami.Ospf.Quantities.Unit
{
    ...
    public abstract class PhysicalUnit
    {
        ...
        {
            if (!Quantity.Equals(unit.Quantity)) return null;
            if (IsAffine || unit.IsAffine) return null;
        ...
        {
            if (!Quantity.Equals(unit.Quantity)) return null;
            var thisRule = ConversionRule;
            var targetRule = unit.ConversionRule;

            if (thisRule is UnitConversionRule.Linear srcLin && targetRule is UnitConversionRule.Linear tgtLin)
            {
                var factor = (srcLin.Scale / tgtLin.Scale)?.Value;
                if (factor == null) return null;
                return value * factor;
            }
            if (thisRule is UnitConversionRule.Linear srcLin2 && targetRule is UnitConversionRule.Affine tgtAff)
            {
                var srcScale = srcLin2.Scale.Value;
                var tgtScale = tgtAff.Scale.Value;
                if (srcScale == null || tgtScale == null) return null;
            ...
            {
                var srcScale = srcAff.Scale.Value;
                var tgtScale = tgtLin2.Scale.Value;
                if (srcScale == null || tgtScale == null) return null;
            ...
            {
                var srcScale = srcAff2.Scale.Value;
                var tgtScale = tgtAff2.Scale.Value;
                if (srcScale == null || tgtScale == null) return null;
        ...
        {
            if (obj is not PhysicalUnit other) return false;
    ...
    public static class PhysicalUnitOperators
    {
        ...
        {
            RequireLinear(unit, "scale");
            var divResult = unit.Scale / scale;
        ...
        public static PhysicalUnit Pow(this PhysicalUnit unit, int index)
        {
            ...
            if (index == 0) return NoneUnit.Instance;
            if (index > 0)
            {
                var result = unit;
            ...
            {
                var result = NoneUnit.Instance.Divide(unit);
                for (int i = 1; i < -index; i++)
