...
public sealed class GurobiLinearSolver : ILinearSolver
{
    ...
    public async Task<Result<SolverOutput, ErrorCode, Error<ErrorCode>>> InvokeAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await InvokeAsync((ILinearTriadModelView)model, solvingStatusCallBack, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        if (solutionAmount <= 1)
        {
            var single = await InvokeAsync(model, solvingStatusCallBack, cancellationToken);
        ...
        var callBack = (_callBack?.Copy() ?? new GurobiLinearSolverCallBack())
            ...
            {
                var count = System.Math.Min((int)solutionAmount, grbModel.Get(GRB.IntAttr.SolCount));
                for (var i = 0; i < count; i++)
                {
                    grbModel.Set(GRB.IntParam.SolutionNumber, i);
                    var thisResults = new List<Flt64>(variables.Count);
                    foreach (var v in variables) { thisResults.Add(new Flt64(v.Get(GRB.DoubleAttr.Xn))); }
        ...
        var result = await impl.InvokeAsync(model);
        return result switch
        {
            Ok<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>> ok =>
    ...
    {
        var tokens = model.Tokens.TokensInSolver;
        var variables = new List<ModelViewVariable>(tokens.Count);
        for (int i = 0; i < tokens.Count; i++)
        {
            var v = tokens[i].Variable;
        ...
        var constraints = ((IMechanismModel<Flt64>)model).Constraints;
        var rowCount = constraints.Count;
        ...
        for (int row = 0; row < rowCount; row++)
        {
            ...
            foreach (var cell in constraint.Lhs)
            {
                if (cell is ILinearCell<Flt64> linearCell)
                {
                    var colIndex = model.Tokens.IndexOf(linearCell.Token);
    ...
    private sealed class GurobiLinearSolverImpl : GurobiSolver
    {
        ...
        {
            var server = _connectionConfig?.Server;
            var password = _connectionConfig?.Password;
            var connectionTime = _connectionConfig?.ConnectionTime;
            ...
            // Step 4: Solve
            var solveResult = await SolveAsync();
            ...
            var statusResult = await AnalyzeStatusAsync();
        ...
        private Result<Success, ErrorCode, Error<ErrorCode>> Dump(ILinearTriadModelView model)
        {
            try
            {
                ...
                foreach (var v in model.Variables)
                {
                    ...
                    if (lb.ToDouble() <= -1e100) lb = new Flt64(-GRB.INFINITY);
                    if (ub.ToDouble() >= 1e100) ub = new Flt64(GRB.INFINITY);
                ...
                for (int i = 0; i < constraints.Size; i++)
                {
                    ...
                    foreach (var cell in constraints.Lhs[i])
                    {
                ...
                foreach (var cell in model.Objective.Cells)
                {
                ...
                var cbResult = _callBack?.ExecIfContainAsync(
        ...
        private async Task<Result<FeasibleSolverOutput<Flt64>, ErrorCode, Error<ErrorCode>>> AnalyzeSolution()
        {
            try
            {
                if (Status.Succeeded())
                {
                    ...
                    foreach (var grbVar in _grbVars)
                    {
                        results.Add(new Flt64(grbVar.Get(GRB.DoubleAttr.X)));
                    }

                    var isMip = GrbModel.Get(GRB.IntAttr.IsMIP) != 0;
                    ...
                    var cbResult = await (_callBack?.ExecIfContainAsync(
...
// 
//         if (cbWhere == GRB.Callback.MIP || cbWhere == GRB.Callback.MIPSOL)
//         {
//             try
//             {
...
//                 var gap = currentObj == Flt64.Zero
//                     ? Flt64.Zero
//                     : (currentObj.Minus(currentBound)).Div(currentObj.Plus(new Flt64(1e-10))).Abs();
// 
