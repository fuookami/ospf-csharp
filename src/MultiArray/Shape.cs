...
public static class ShapeExtensions {
    ...
    public static int? ActualIndex(this IShape shape, int dimension, int index) {
        if (dimension < 0 || dimension >= shape.Dimension) {
            return null;
        }
    ...
    public static IReadOnlyList<DummyIndexIterator> DummyToIteratorVector(this IShape shape, IReadOnlyList<DummyIndex> dummyVector) => dummyVector.Select((d, i) => d.IteratorOf(shape, i)).ToList();

    /// <summary>将虚拟向量转换为映射向量 / Convert dummy vector to map vector</summary>
    public static IReadOnlyList<MapIndex> DummyToMapVector(this IShape shape, IReadOnlyList<DummyIndex> dummyVector) => dummyVector.Select(d => (MapIndex)new MapIndex.Dummy(d)).ToList();
    ...
    public static IReadOnlyList<DummyIndexIterator> MapToIteratorVector(this IShape shape, IReadOnlyList<MapIndex> mapVector) {
        return mapVector.Select((m, i) => {
            if (m is MapIndex.Dummy d) {
                return d.DummyValue.IteratorOf(shape, i);
            }

            if (m is MapIndex.Map mp) {
                return (DummyIndexIterator)new DummyIndexIterator.Continuous(0, shape[mp.Index]);
            }
    ...
    public static Result<IReadOnlyList<DummyIndex>, ErrorCode, Error<ErrorCode>> DummyVector(this IShape shape, params object[] v) => shape.DummyVectorSafe(v);
    ...
    public static IReadOnlyList<DummyIndex> DummyVectorUnchecked(this IShape shape, params object[] v) => shape.DummyVectorOrNull(v) ?? (IReadOnlyList<DummyIndex>)Array.Empty<DummyIndex>();

    /// <summary>尝试创建虚拟向量 / Try create dummy vector</summary>
    public static IReadOnlyList<DummyIndex>? DummyVectorOrNull(this IShape shape, params object[] v) => shape.DummyVectorSafe(v).Value;
}
...
public sealed record Shape1 : IShape {
    ...
    public RetInt Index(int[] vector) {
        if (vector.Length != 1) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Dimension mismatch: expected 1, got {vector.Length}");
        }

        if (vector[0] < 0 || vector[0] >= _d1) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Shape index out of bounds: dimension 0 length is {_d1}, got {vector[0]}");
        }
    ...
    public RetIntArray Vector(int index) {
        if (index < 0 || index >= _d1) {
            return new FailedIntArray(ErrorCode.IllegalArgument, $"Linear index out of bounds: index {index} is outside shape size {_d1}");
        }
...
public sealed record Shape2 : IShape {
    ...
    public RetInt Index(int[] vector) {
        if (vector.Length != 2) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Dimension mismatch: expected 2, got {vector.Length}");
        }
        ...
        if (vector[1] < 0 || vector[1] >= _d2) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Shape index out of bounds: dimension 1 length is {_d2}, got {vector[1]}");
        }
    ...
    /// <inheritdoc/>
    public RetIntArray Vector(int index) {
        int total = _totalSize.Value;
        if (index < 0 || index >= total) {
            return new FailedIntArray(ErrorCode.IllegalArgument, $"Linear index out of bounds: index {index} is outside shape size {total}");
        }
...
public sealed record Shape3 : IShape {
    ...
    public RetInt Index(int[] vector) {
        if (vector.Length != 3) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Dimension mismatch: expected 3, got {vector.Length}");
        }
        ...
        if (vector[2] < 0 || vector[2] >= _d3) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Shape index out of bounds: dimension 2 length is {_d3}, got {vector[2]}");
        }
...
public sealed record Shape4 : IShape {
    ...
    public RetInt Index(int[] vector) {
        if (vector.Length != 4) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Dimension mismatch: expected 4, got {vector.Length}");
        }
        ...
        if (vector[3] < 0 || vector[3] >= _d4) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Shape index out of bounds: dimension 3 length is {_d4}, got {vector[3]}");
        }
...
public sealed record DynShape : IShape {
    ...
    public RetInt Index(int[] vector) {
        if (Dimension != vector.Length) {
            return new FailedRetInt(ErrorCode.IllegalArgument, $"Dimension mismatch: expected {Dimension}, got {vector.Length}");
        }
