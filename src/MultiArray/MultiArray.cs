...
namespace Fuookami.Ospf.MultiArray
{
        ...
        where S : IShape
    {
        ...
        public static class Factory
        {
            ...
            public static Result<MultiArray<T, S>, ErrorCode, Error<ErrorCode>> New(S shape)
            {
            ...
            {
                var defaultValue = DefaultValueRegistry.GetDefault(typeof(T));
            ...
            public static MultiArray<T, S> NewWith(S shape, T value)
            {
                return new MultiArray<T, S>(shape, (_, _) => value);
            }

            /// <summary>使用生成器创建 / Create with generator</summary>
            public static MultiArray<T, S> NewBy(S shape, Func<int, int[], T> generator)
            {
                return new MultiArray<T, S>(shape, generator);
            }
                ...
                S shape, IReadOnlyList<T> list, AccessOrder accessOrder = AccessOrders.Default)
            {
                ...
                var reordered = ReorderToStorageOrder(shape, list, accessOrder);
                return new Ok<MultiArray<T, S>, ErrorCode, Error<ErrorCode>>(
        ...
        public MultiArray<T, DynShape> ToStorageOrder(StorageOrder order)
        {
            ...
            {
                var dims = Enumerable.Range(0, Shape.Dimension).Select(i => Shape[i]).ToArray();
                var newShape = DynShape.WithOrder(dims, order);
                return new MultiArray<T, DynShape>(newShape, (i, _) => DataList[i]);
            }

            var dimensions = Enumerable.Range(0, Shape.Dimension).Select(i => Shape[i]).ToArray();
            var targetShape = DynShape.WithOrder(dimensions, order);

            return new MultiArray<T, DynShape>(targetShape, (newLinearIndex, _) =>
            {
                var vector = targetShape.VectorUnchecked(newLinearIndex);
                var oldLinearIndex = Shape.IndexUnchecked(vector);
                return DataList[oldLinearIndex];
            });
        }

        /// <summary>重塑数组 / Reshape array</summary>
        public MultiArray<T, NS> Reshape<NS>(NS newShape, T fillValue) where NS : IShape
        {
            return new MultiArray<T, NS>(newShape, (i, _) => i < Count ? DataList[i] : fillValue);
        }

        /// <summary>转换为可变数组 / Convert to mutable array</summary>
        public MutableMultiArray<T, S> ToMutable()
        {
            return new MutableMultiArray<T, S>(Shape, (i, _) => DataList[i]);
        }
        ...
        private static T[] ReorderToStorageOrder(IShape shape, IReadOnlyList<T> list, AccessOrder accessOrder)
        {
            ...
            var inputIndex = 0;
            foreach (var vector in new MultiIndexSequence(shape, accessOrder))
            {
        ...
        where S : IShape
    {
        ...
        {
            /// <summary>使用默认值安全创建 / Safely create with default values</summary>
            public static Result<MutableMultiArray<T, S>, ErrorCode, Error<ErrorCode>> New(S shape)
            {
                return NewSafe(shape);
            }
            ...
            /// <summary>使用指定值创建 / Create with specified value</summary>
            public static MutableMultiArray<T, S> NewWith(S shape, T value)
            {
                return new MutableMultiArray<T, S>(shape, (_, _) => value);
            }

            /// <summary>使用生成器创建 / Create with generator</summary>
            public static MutableMultiArray<T, S> NewBy(S shape, Func<int, int[], T> generator)
            {
                return new MutableMultiArray<T, S>(shape, generator);
            }
        ...
        public MultiArray<T, S> ToImmutable()
        {
            return new MultiArray<T, S>(Shape, (i, _) => DataList[i]);
        }
