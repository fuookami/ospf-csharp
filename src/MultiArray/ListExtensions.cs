...
namespace Fuookami.Ospf.MultiArray
{
    ...
    public static class List2Extensions
    {
        ...
        public static IEnumerable<T> GetColumn<T>(this IReadOnlyList<IReadOnlyList<T>> list, DummyIndex.All allIndex, int j)
        {
            return list.Select(row => j >= 0 && j < row.Count ? row[j] : default!);
        }

        /// <summary>获取指定行的所有列 / Get all columns from a row</summary>
        public static IEnumerable<T> GetRow<T>(this IReadOnlyList<IReadOnlyList<T>> list, int i, DummyIndex.All allIndex)
        {
            return i >= 0 && i < list.Count ? list[i] : Enumerable.Empty<T>();
        }

        /// <summary>获取所有元素 / Get all elements</summary>
        public static IEnumerable<T> GetAll<T>(this IReadOnlyList<IReadOnlyList<T>> list, DummyIndex.All allIndex1, DummyIndex.All allIndex2)
        {
            return list.SelectMany(row => row);
        }
    }
    ...
    public static class List3Extensions
    {
        ...
        public static IEnumerable<T> GetFromAllLayers<T>(this IReadOnlyList<IReadOnlyList<IReadOnlyList<T>>> list, DummyIndex.All allIndex, int j, int k)
        {
            ...
            {
                if (j < 0 || j >= layer.Count) return default!;
                var row = layer[j];
        ...
        {
            if (i < 0 || i >= list.Count) return Enumerable.Empty<T>();
        ...
        {
            if (i < 0 || i >= list.Count) return Enumerable.Empty<T>();
            if (j < 0 || j >= list[i].Count) return Enumerable.Empty<T>();
            return list[i][j];
        }
    }
