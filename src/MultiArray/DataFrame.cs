...
public class DataFrame<T> : ICollection<IReadOnlyList<T?>> {
    ...
    public int? GetColumnIndex(string name) => _columnIndex.TryGetValue(name, out int idx) ? idx : null;
    ...
    public Result<T?, ErrorCode, Error<ErrorCode>> GetByNameSafe(int row, string columnName) {
        if (!_columnIndex.TryGetValue(columnName, out int col)) {
    ...
    public MultiArray<NullableValue<T>, Shape2> ToNullableMultiArray() {
        MutableMultiArray<NullableValue<T>, Shape2> array = MutableMultiArray<NullableValue<T>, Shape2>.Factory.NewWith(
    ...
    public DataFrame<T> SubDataFrame(System.Range rows, System.Range cols) {
        int rowStart = rows.Start.IsFromEnd ? NRows - rows.Start.Value : rows.Start.Value;
        int rowEnd = rows.End.IsFromEnd ? NRows - rows.End.Value : rows.End.Value;
        int colStart = cols.Start.IsFromEnd ? NCols - cols.Start.Value : cols.Start.Value;
        int colEnd = cols.End.IsFromEnd ? NCols - cols.End.Value : cols.End.Value;
    ...
    public Result<DataFrame<T>, ErrorCode, Error<ErrorCode>> SelectSafe(params string[] columnNames) {
        ...
        foreach (string name in columnNames) {
            if (!_columnIndex.TryGetValue(name, out int col)) {
    ...
    public Result<IReadOnlyDictionary<string, IReadOnlyList<T?>>, ErrorCode, Error<ErrorCode>> ToMapSafe() {
        ...
        foreach (string name in ColumnNames) {
            Result<IReadOnlyList<T?>, ErrorCode, Error<ErrorCode>> result = GetColumnByNameSafe(name);
    ...
    public void CopyTo(IReadOnlyList<T?>[] array, int arrayIndex) {
        int idx = arrayIndex;
        foreach (IReadOnlyList<T?> row in this) {
            array[idx++] = row;
        }
    }
    ...
    public static DataFrame<T> FromMap(IReadOnlyDictionary<string, IReadOnlyList<T?>> data) {
        ...
        int nrows = data.Values.FirstOrDefault()?.Count ?? 0;
        int ncols = columnNames.Count;

        var df = new DataFrame<T>(nrows, ncols, columnNames);
        for (int colIdx = 0; colIdx < columnNames.Count; colIdx++) {
            IReadOnlyList<T?> values = data[columnNames[colIdx]];
...
public class DataFrameBuilder<T> {
    ...
    public void Rows(IReadOnlyList<IReadOnlyList<T?>> values) {
        foreach (IReadOnlyList<T?> row in values) {
