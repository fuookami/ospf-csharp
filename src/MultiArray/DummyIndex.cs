    ...
    public abstract record DummyIndex
    {
        ...
        public int LenOf(IShape shape, int dimension)
        {
            switch (this)
            {
                ...
                case Range r:
                {
                    ...
                    var start = ActualBound(r.RangeValue.Start(), len) ?? 0;
                    var end = ActualBound(r.RangeValue.End(), len) ?? len;
        ...
        public DummyIndexIterator IteratorOf(IShape shape, int dimension)
        {
            switch (this)
            {
                case Index idx:
                {
                    var actualIdx = shape.ActualIndex(dimension, idx.Value);
    ...
    public abstract record DummyIndexIterator
    {
        ...
        public int? Get(int i)
        {
            switch (this)
            {
                ...
                case Continuous c:
                {
                    var length = Math.Max(0, c.End - c.Start);
