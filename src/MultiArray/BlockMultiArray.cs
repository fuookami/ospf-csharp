...
public class BlockMultiArray<T, S> : ICollection<T>
    ...
    public T? Get(params int[] indices) => _blocks.TryGetValue(IndexKey.Transient(indices), out T? value) ? value : default;

    /// <summary>通过向量索引设置元素 / Set element by vector index</summary>
    public void Set(int[] indices, T value) => _blocks[IndexKey.Persistent(indices)] = value;
    ...
    public T GetOrSet(int[] indices, Func<T> defaultValue) {
        ...
        if (_blocks.TryGetValue(key, out T? existed)) {
            return existed;
        }
    ...
    public T? Remove(int[] indices) {
        var key = IndexKey.Transient(indices);
        if (_blocks.TryGetValue(key, out T? value)) {
            _blocks.Remove(key);
            return value;
        }
        return default;
    ...
    public MultiArray<T, S> ToMultiArray(T defaultValue) {
        MutableMultiArray<T, S> array = MutableMultiArray<T, S>.Factory.NewWith(Shape, defaultValue);
        foreach ((IndexKey? key, T? value) in _blocks) {
    ...
    public static BlockMultiArray<T, S> FromMultiArray(MultiArray<T, S> array, Func<T, bool>? filter = null) {
        ...
        for (int i = 0; i < array.Shape.Size; i++) {
            int[] vector = array.Shape.VectorUnchecked(i);
            T value = array[vector];
