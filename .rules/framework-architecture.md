...
本规范适用于 `Fuookami.Ospf.Framework.*` 下以优化建模为核心的 framework 模块，包括但不限于 CSP1D、BPP3D、Gantt Scheduling 等领域框架。
...
1. 对外领域模型、配置和结果优先使用泛型数值抽象（如 `where T : struct, INumber<T>`）。
2. 宽度、长度、重量、面积、需求、产出、余料、容量等领域量应优先使用明确的物理量类型表达。
3. `double`、`decimal`、无单位裸值只能作为 solver adapter、registration、extraction 或低层数学接口的实现细节。
